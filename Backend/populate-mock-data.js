...
console.log('Starting mock data population script...');
...
async function createMockUsers(count) {
  ...
  for (let i = 0; i < count; i++) {
    ...
    do {
      const baseUsername = usernames[i] || `${firstName}${lastName}${getRandomInt(100, 999)}`;
      username = attempts > 0 ? `${baseUsername}_${attempts}` : baseUsername;
      attempts++;
    } while (existingUsernames.has(username) && attempts < 10);
    
    existingUsernames.add(username); // Add to set to prevent duplicates in this run
    
    const userData = {
      username,
      email: `${username.toLowerCase().replace(/[^a-z0-9]/g, '')}@example.com`,
      googleId: `fake_google_id_${Date.now()}_${i}`,
      authProvider: 'google',
      ...
      website: Math.random() > 0.5 ? `https://${username.toLowerCase().replace(/[^a-z0-9]/g, '')}.com` : '',
    ...
    try {
      ...
      console.log(`Skipped user ${username} (likely duplicate):`, error.message);
    }
  }
...
async function createMockTournaments(creatorUser, users) {
  ...
  for (let i = 0; i < 15; i++) {
    ...
    const tournamentData = {
      ...
      type: getRandom(['artist', 'producer', 'aux']),
