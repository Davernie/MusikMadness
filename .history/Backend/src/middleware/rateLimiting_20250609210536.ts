...
// SECURITY IMPROVEMENT: Progressive delay middleware to slow down brute force attacks
// without locking out legitimate users
...
export const progressiveAuthDelay = (req: Request, res: Response, next: Function) => {
  ...
  for (const [ip, data] of authAttemptCounts.entries()) {
    if (now - data.firstAttempt > windowMs) {
      authAttemptCounts.delete(ip);
    }
  }
  
  const attemptData = authAttemptCounts.get(clientIP);
  
  if (attemptData) {
    // Progressive delay: 0s, 1s, 2s, 4s, 8s, 16s, 32s (max)
    const delayMs = Math.min(Math.pow(2, attemptData.count - 1) * 1000, 32000);
    
    if (delayMs > 0) {
      console.log(`⏱️  Progressive delay: ${delayMs}ms for IP ${clientIP} (attempt ${attemptData.count})`);
...
export const trackAuthFailure = (clientIP: string) => {
  const now = Date.now();
  const existing = authAttemptCounts.get(clientIP);
  
  if (existing) {
    existing.count++;
  } else {
    authAttemptCounts.set(clientIP, { count: 1, firstAttempt: now });
  }
};

// Reset auth attempts on successful login
export const resetAuthAttempts = (clientIP: string) => {
  authAttemptCounts.delete(clientIP);
};
