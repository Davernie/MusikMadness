...
import { generalLimiter, logSuspiciousActivity } from './middleware/rateLimiting';
...
// Apply rate limiting only in production for testing purposes
...
app.use((req, res, next) => {  // Security headers for HTTPS and SSL/TLS
  ...
  // Note: X-Frame-Options DENY would block YouTube iframes, so we rely on CSP frame-src instead
...
const startServer = async () => {
  try {
    ...
    // Start streamer status monitoring
    const streamerStatusService = (await import('./services/streamerStatusService.js')).default;
