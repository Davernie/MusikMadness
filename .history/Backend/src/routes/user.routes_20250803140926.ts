...
router.put(
  ...
  [
    ...
    body('firstName')
      ...
      .isString()
      .isLength({ max: 50 })
      .withMessage('First name must be a string with max 50 characters'),
    body('lastName')
      .optional()
      .isString()
      .isLength({ max: 50 })
      .withMessage('Last name must be a string with max 50 characters'),
    body('bio')
      .optional()
      .isLength({ max: 500 })
    ...
    body('website')
      ...
      .custom((value) => {
        ...
        // Use a simple URL validation that's more permissive
...
router.get('/raw-image/:id', async (req, res) => {
  try {
    ...
    res.send(htmlResponse);  } catch (error) {
    console.error('Raw image debug error:', error);
    res.status(500).json({ message: 'Server error', error: String(error) });  }
});

// @route   GET /api/users/:id/tournaments/created
// @desc    Get tournaments created by user
// @access  Public
router.get('/:id/tournaments/created', async (req, res) => {
  try {
    const userId = req.params.id;
...
// @route   GET /api/users/:id/tournaments/joined
// @desc    Get tournaments user has joined/participated in
// @access  Public
router.get('/:id/tournaments/joined', async (req, res) => {
  try {
    const userId = req.params.id;
