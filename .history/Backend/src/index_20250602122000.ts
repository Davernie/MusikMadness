...
// This should come AFTER API routes but BEFORE the SPA fallback.
const frontendDistPath = path.join(__dirname, '../../Frontend/dist');
app.use(express.static(frontendDistPath));

// Enhanced health check route (can be before or after static, but before SPA fallback)
...
// serve the main index.html. This allows client-side routing to take over.
// This MUST be one of the LAST GET route handlers.
app.get('*', (req: Request, res: Response, next: NextFunction) => {
  // Check if the request looks like an API call or a request for a specific file with an extension
  if (req.path.startsWith('/api/') || req.path.includes('.')) {
    // If it's an API call not handled by earlier routes, or a file not found by express.static,
    // let it proceed to the 404 handler (or next error handler).
    return next();
  }
  
  // For all other GET requests, it's likely a client-side route.
  res.sendFile(path.join(frontendDistPath, 'index.html'), (err) => {
    if (err) {
      // If there's an error sending index.html (e.g., file not found, though unlikely if build is correct)
