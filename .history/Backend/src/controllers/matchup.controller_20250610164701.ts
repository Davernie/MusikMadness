...
export const getMatchupsByTournament = async (req: Request, res: Response) => {
  try {
    ...
    // Map streamingSource to audioType for frontend compatibility
    const mapSubmissionFields = (submission: any) => {
      if (!submission) return submission;
      
      const mapped = submission.toObject ? submission.toObject() : submission;
      
      // Debug logging to see what's in the submission
      console.log('DEBUG - Original submission data:', JSON.stringify({
        id: mapped._id,
        streamingSource: mapped.streamingSource,
        youtubeUrl: mapped.youtubeUrl,
        soundcloudUrl: mapped.soundcloudUrl,
        soundcloudTrackId: mapped.soundcloudTrackId,
        songTitle: mapped.songTitle
      }, null, 2));
      
      // Map streamingSource to audioType for frontend compatibility
      if (mapped.streamingSource) {
        mapped.audioType = mapped.streamingSource;
      }
      
      console.log('DEBUG - Mapped audioType:', mapped.audioType);
      
      return mapped;
    };

    // Map all matchups with field mapping
    const mappedMatchups = matchups.map(matchup => ({
      ...matchup.toObject(),
      track1: mapSubmissionFields(matchup.track1),
      track2: mapSubmissionFields(matchup.track2)
    }));

    res.json(mappedMatchups);
  } catch (error) {
...
export const getMatchupById = async (req: Request, res: Response) => {
  try {
    ...
    // Create the response object with mapped fields
    const responseMatchup = {
      ...matchup.toObject(),
      track1: mapSubmissionFields(matchup.track1),
      track2: mapSubmissionFields(matchup.track2),
      winner: mapSubmissionFields(matchup.winner)
    };

    res.json(responseMatchup);
  } catch (error) {
