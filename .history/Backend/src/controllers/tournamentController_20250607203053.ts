...
export const getMatchupById = async (req: Request, res: Response) => {
  try {
    ...
    // Optimized: Only fetch tournament with minimal data needed
    const tournament = await Tournament.findById(tournamentId)
      .select('name generatedBracket')
    ...
    // Collect all participant IDs for batch query
    const participantIds = [
      matchup.player1.participantId,
      matchup.player2.participantId
    ].filter(Boolean);

    // Batch fetch users and submissions
    const [users, submissions] = await Promise.all([
      participantIds.length > 0 
        ? User.find({ _id: { $in: participantIds } })
            .select('_id username profilePicture.contentType')
            .lean()
        : Promise.resolve([]),
      participantIds.length > 0
        ? Submission.find({ 
            ...
            .select('user songTitle description streamingSource youtubeVideoId youtubeUrl youtubeThumbnail youtubeDuration soundcloudUrl soundcloudTrackId soundcloudArtwork soundcloudDuration soundcloudUsername r2Key r2Url songFilePath originalFileName mimetype')
            .lean()
        : Promise.resolve([])
    ]);
    ...
    const submissionMap = new Map(submissions.map(sub => [sub.user.toString(), sub]));

    // Helper to prepare participant data (optimized)
    const prepareCompetitorData = (participantId: string | null, displayName: string) => {
      if (!participantId) {
        return {
          id: null,
          name: displayName,
      ...
      if (user && user.profilePicture && user.profilePicture.contentType) {
        profilePictureUrl = `${baseUrl}/api/users/${user._id}/profile-picture`;
      }

      let submissionDetails = null;
      if (submission) {
        // For matchup details, we don't need the heavy presigned URL generation
        // That will be handled by the stream-urls endpoint
        let audioUrl = '';
        ...
        submissionDetails = {
          ...
          audioUrl,
          streamUrl: null, // Will be provided by stream-urls endpoint
      ...
      return {
        id: user ? user._id.toString() : participantId,
        name: user ? user.username : displayName,
        artist: user ? user.username : displayName,
        profilePictureUrl,
        submission: submissionDetails,
      };
    };

    const competitor1 = prepareCompetitorData(
      matchup.player1.participantId ? matchup.player1.participantId.toString() : null, 
      matchup.player1.displayName
    );
    const competitor2 = prepareCompetitorData(
      matchup.player2.participantId ? matchup.player2.participantId.toString() : null, 
      matchup.player2.displayName
    );
    ...
    const responseMatchup = {
      ...
      player1: {
        ...competitor1,
        score: matchup.player1.score,
      },
      player2: {
        ...competitor2,
        score: matchup.player2.score,
      },
