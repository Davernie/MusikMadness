  ...
  try {
    ...
    const prepareCompetitorData = async (participantId: string | null, displayName: string) => {
      ...
      const submission = await Submission.findOne({
        tournament: tournamentId,
        user: participantId,
      }).select('songTitle description songFilePath r2Key r2Url originalFileName mimetype streamingSource youtubeVideoId youtubeUrl youtubeThumbnail youtubeDuration soundcloudTrackId soundcloudUrl soundcloudArtwork soundcloudDuration soundcloudUsername'); // Include YouTube and SoundCloud fields
