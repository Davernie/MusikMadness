...
import { isValidSoundCloudUrl, fetchSoundCloudTrackData, getSoundCloudArtwork, soundCloudDurationToSeconds, getSoundCloudEmbedUrl, getSoundCloudTrackId } from '../utils/soundcloud';
...
export const joinTournament = async (req: Request, res: Response) => {
  try {
    ...
    }    // 4. Create new submission with R2 data or YouTube data
    const submissionData: any = {
      tournament: tournamentId,
      user: userId,
      songTitle,
      description,
      streamingSource: streamingSource || 'upload'
    };
    ...
    if (streamingSource === 'youtube') {
      ...
      try {
        ...
        if (!soundcloudData) {
          return res.status(400).json({ message: 'Could not fetch SoundCloud track data. Please check the URL and ensure the track is public.' });
        }

        submissionData.soundcloudUrl = soundcloudUrl;
        submissionData.soundcloudTrackId = soundcloudData.id;
        submissionData.soundcloudArtwork = getSoundCloudArtwork(soundcloudData.artwork_url, 'large');
        submissionData.soundcloudDuration = soundCloudDurationToSeconds(soundcloudData.duration);
        ...
        console.error('Error processing SoundCloud submission:', error);
        return res.status(400).json({ message: 'Could not process SoundCloud track. Please ensure the track is public and accessible.' });
      }

      // Set empty values for file-related fields
      submissionData.originalFileName = `${submissionData.soundcloudTrackId}.soundcloud`;
    ...
    await tournament.save();    // Determine audio/video URL for response
    let mediaUrl: string;
    ...
    res.status(201).json({
      ...
      submission: {
        ...
        streamingSource: newSubmission.streamingSource,
        mediaUrl,
        storageType,        ...(streamingSource === 'youtube' && {
  ...
  try {
    ...
    const prepareCompetitorData = async (participantId: string | null, displayName: string) => {
      ...
      const submission = await Submission.findOne({
        tournament: tournamentId,
        user: participantId,
      }).select('songTitle description songFilePath r2Key r2Url originalFileName mimetype streamingSource youtubeVideoId youtubeUrl youtubeThumbnail youtubeDuration soundcloudTrackId soundcloudUrl soundcloudArtwork soundcloudDuration soundcloudUsername'); // Include YouTube and SoundCloud fields      let submissionDetails = null;
      if (submission) {
        let audioUrl: string;
        let streamUrl: string | null = null;
        let audioType: 'r2' | 'local' | 'youtube' | 'soundcloud' = 'local';
        
        // Handle YouTube submissions
        if (submission.streamingSource === 'youtube') {
          audioUrl = submission.youtubeUrl || '';
          streamUrl = submission.youtubeUrl;
          audioType = 'youtube';
        }
        // Handle SoundCloud submissions
        else if (submission.streamingSource === 'soundcloud') {
          audioUrl = submission.soundcloudUrl || '';
          streamUrl = submission.soundcloudUrl;
          audioType = 'soundcloud';
        }
        // Prefer R2 URL if available, with presigned URL for better security
        else if (submission.r2Key && submission.r2Url) {
          try {
        ...
        submissionDetails = {
          ...
          audioType, // Indicates whether it's from R2, local storage, YouTube, or SoundCloud
          streamingSource: submission.streamingSource,
          // YouTube-specific fields
          ...(submission.streamingSource === 'youtube' && {
            youtubeVideoId: submission.youtubeVideoId,
            youtubeThumbnail: submission.youtubeThumbnail,
            youtubeDuration: submission.youtubeDuration,
            embedUrl: getYouTubeEmbedUrl(submission.youtubeVideoId!)
          }),
          // SoundCloud-specific fields
          ...(submission.streamingSource === 'soundcloud' && {
  ...
  try {
    ...
    const getParticipantStreamUrl = async (participantId: string | null) => {
      ...
      const submission = await Submission.findOne({
        tournament: tournamentId,
        user: participantId,
      }).select('r2Key r2Url songFilePath streamingSource youtubeVideoId youtubeUrl youtubeThumbnail youtubeDuration soundcloudTrackId soundcloudUrl soundcloudArtwork soundcloudDuration soundcloudUsername songTitle');
      ...
      if (submission.streamingSource === 'soundcloud') {
        if (!submission.soundcloudTrackId || !submission.soundcloudUrl) {
          return null;
        }
