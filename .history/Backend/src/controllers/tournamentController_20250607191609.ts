  ...
  try {
    ...
    const getParticipantStreamUrl = async (participantId: string | null) => {
      ...
      if (submission.streamingSource === 'soundcloud') {
        ...
        // Check SoundCloud rate limit status
        const rateLimitStatus = await getSoundCloudRateStatus(submission.soundcloudUrl);
        if (rateLimitStatus && rateLimitStatus.limitExceeded) {
          return res.status(429).json({ message: 'SoundCloud rate limit exceeded. Please try again later.' });
        }
