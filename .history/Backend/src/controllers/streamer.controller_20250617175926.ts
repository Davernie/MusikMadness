...
export const getStreamers = async (req: Request, res: Response) => {
  try {
    ...
    const streamersData = streamers.map(streamer => {
      const streamerObj: any = streamer.toObject();
      
      // If userId is populated, move it to a 'user' field for frontend
      if (streamerObj.userId && typeof streamerObj.userId === 'object' && (streamerObj.userId as any).username) {
        streamerObj.user = streamerObj.userId;
        streamerObj.userId = (streamerObj.userId as any)._id;
      }
    ...
    streamersData.forEach((streamer: any) => {
