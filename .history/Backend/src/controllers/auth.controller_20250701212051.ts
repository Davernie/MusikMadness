...
export const signup = async (req: Request, res: Response) => {
  try {
    ...
    const { username, email, password, bio, streaming } = req.body;

    // Parse streaming data if it's a string (from FormData)
    let streamingData;
    if (streaming) {
      try {
        streamingData = typeof streaming === 'string' ? JSON.parse(streaming) : streaming;
      } catch (error) {
        return res.status(400).json({ 
          message: 'Invalid streaming data format',
          fieldErrors: { streaming: ['Invalid streaming data'] }
        });
      }
    }
    ...
    // Handle streaming data if provided
    if (streamingData) {
      user.streaming = {
        isStreamer: streamingData.isStreamer || false,
        wantsFeatured: streamingData.wantsFeatured || false,
        preferredPlatform: streamingData.preferredPlatform || 'youtube',
        streamingAccount: streamingData.streamingAccount || '',
        // Set defaults for other streaming fields
        isLive: false,
        streamTitle: '',
        streamDescription: '',
        viewerCount: 0,
