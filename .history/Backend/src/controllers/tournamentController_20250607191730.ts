...
import { isValidSoundCloudUrl, fetchSoundCloudTrackData, getSoundCloudArtwork, soundCloudDurationToSeconds, getSoundCloudEmbedUrl } from '../utils/soundcloud';
import { isSoundCloudRateLimited, getSoundCloudRateStatus } from '../utils/soundcloudRateLimit';
...
export const joinTournament = async (req: Request, res: Response) => {
  try {
    ...
    if (streamingSource === 'youtube') {
      ...
      // Try to fetch SoundCloud track metadata
      try {
  ...
  try {
    ...
    const prepareCompetitorData = async (participantId: string | null, displayName: string) => {
      ...
      const submission = await Submission.findOne({
        tournament: tournamentId,
        user: participantId,
      }).select('songTitle description songFilePath r2Key r2Url originalFileName mimetype streamingSource youtubeVideoId youtubeUrl youtubeThumbnail youtubeDuration'); // Include YouTube fields
  ...
  try {
    ...
    const getParticipantStreamUrl = async (participantId: string | null) => {
      ...
      if (submission.streamingSource === 'soundcloud') {
        ...
        // Check if SoundCloud is rate limited
        if (isSoundCloudRateLimited()) {
          const rateStatus = getSoundCloudRateStatus();
          console.warn('🚨 SoundCloud rate limited, returning fallback response');
          ...
          return {
            ...
            rateLimited: true,
            rateLimitMessage: `SoundCloud play limit reached. Try again ${
              rateStatus.resetTime ? `after ${rateStatus.resetTime.toLocaleString()}` : 'later'
            }`
          };
        }
...
// New endpoint to check SoundCloud rate limit status
export const getSoundCloudStatus = async (req: Request, res: Response) => {
  try {
    const rateStatus = getSoundCloudRateStatus();
    
    res.json({
      isRateLimited: rateStatus.isLimitReached,
      playsRemaining: rateStatus.playsRemaining,
      resetTime: rateStatus.resetTime,
      message: rateStatus.isLimitReached 
        ? `SoundCloud play limit reached. Try again ${
            rateStatus.resetTime ? `after ${rateStatus.resetTime.toLocaleString()}` : 'later'
          }`
        : 'SoundCloud is available'
    });
  } catch (error) {
    console.error('Error checking SoundCloud status:', error);
    res.status(500).json({ message: 'Error checking SoundCloud status' });
  }
};
