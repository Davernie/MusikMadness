...
export const getMatchupById = async (req: Request, res: Response) => {
  try {
    ...
    const matchup = await Matchup.findById(matchupId)
      .populate({ path: 'track1', model: Track }) // Explicitly specify model for population
      .populate({ path: 'track2', model: Track }) // Explicitly specify model for population
      .populate({ path: 'winner', model: Track }); // Explicitly specify model for population
