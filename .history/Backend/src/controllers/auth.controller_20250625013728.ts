...
export const googleLogin = async (req: Request, res: Response) => {
  try {
    ...
    // Find or create user
    const user = await googleAuthService.findOrCreateGoogleUser(googleUserInfo);
    ...
    // Construct profilePictureUrl - prioritize Google profile picture for Google users
    let profilePictureUrl = null;
    ...
    if (error.message === 'Account already exists with different provider') {
      return res.status(409).json({ 
