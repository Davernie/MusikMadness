...
export const getStreamers = async (req: Request, res: Response) => {
  try {
    ...
    console.log(`🔍 DEBUG Backend: Found ${streamers.length} streamers`);
    streamers.forEach(streamer => {
      console.log(`🔍 DEBUG Backend: ${streamer.name}:`, {
        userId: streamer.userId ? (typeof streamer.userId === 'string' ? streamer.userId : (streamer.userId as any)._id) : 'NONE',
        userPopulated: !!(streamer.userId as any)?.username,
        username: (streamer.userId as any)?.username || 'NO USERNAME'
      });
    });
