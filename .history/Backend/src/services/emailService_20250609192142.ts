...
class EmailService {
    ...
    this.transporter.verify((error) => {
      if (error) {
        ...
        if (error.code === 'EAUTH') {
          ...
          console.log('     EMAIL_USER=musikmadnessofficial@gmail.com');
          console.log('     EMAIL_PASS=<16-char-gmail-app-password>');
          console.log('  3. Generate new Gmail App Password: https://myaccount.google.com/apppasswords');
          console.log('  4. Redeploy your service after updating env vars');
        }
  ...
  public generateVerificationToken(): string {
    ...
    try {
      ...
      console.log(`🔗 Generated verification URL for ${process.env.NODE_ENV || 'development'}: ${verificationUrl}`);
    ...
    try {
      ...
      console.log(`🔗 Generated password reset URL for ${process.env.NODE_ENV || 'development'}: ${resetUrl}`);
