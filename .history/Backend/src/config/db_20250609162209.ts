...
export const connectDB = async (): Promise<void> => {
  try {    // OPTIMIZED for MongoDB M0 (Free Tier) - Limited to 100 total connections
    await mongoose.connect(MONGODB_URI, {
      ...
      // Set default query timeout - REDUCED for M0 free tier
    mongoose.set('maxTimeMS', 10000); // Reduced from 20s to 10s
      console.log('MongoDB connected successfully with M0 (Free Tier) optimized settings');
    console.log(`Max pool size: ${5} connections (optimized for M0 free tier)`);
  } catch (error) {
