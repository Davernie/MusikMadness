...
export const getYouTubeThumbnail = (
  videoId: string,
  quality: 'default' | 'mq' | 'hq' | 'sd' | 'maxres' = 'hq' // Default to hq for high quality
): string => {
...
export const fetchYouTubeVideoData = async (videoId: string): Promise<{
  ...
  if (!apiKey) {
    ...
    return {
      ...
      thumbnail: getYouTubeThumbnail(videoId, 'hq'), // Use corrected utility with 'hq'
  ...
  try {
    ...
    return {
      ...
      duration,
      // Prefer API's high quality thumbnail if available, otherwise use our corrected utility
      thumbnail: video.snippet.thumbnails?.high?.url || getYouTubeThumbnail(videoId, 'hq'),
    ...
    // Fallback if API call fails or returns unexpected structure
    return {
        title: 'YouTube Video (Error Fetching)',
        duration: 0,
        thumbnail: getYouTubeThumbnail(videoId, 'hq'), // Use corrected utility with 'hq'
