...
const generateTestUsers = async (tournamentSize = null) => {
  ...
  try {
    ...
    // Check if 64 users already exist
    ...
    if (existingUsersCount >= 64) {
      console.log('✅ 64 test users already exist, retrieving them...');
    ...
    // Create tournament with specified number of participants (or all 64 if not specified)
    const participantCount = tournamentSize || 64;
    const selectedParticipants = userIds.slice(0, participantCount);
    
    console.log(`Creating tournament with ${participantCount} participants out of 64 available users...`);
    
    // Use the first user as the creator
    const creatorId = selectedParticipants[0];
    
    const tournamentData = {
      name: `${participantCount}-Player Music Madness Championship`,
      game: "Electronic", // You can change this to match your preferred genre
      startDate: new Date(Date.now() + 24 * 60 * 60 * 1000), // Tomorrow
      endDate: new Date(Date.now() + 14 * 24 * 60 * 60 * 1000), // 2 weeks from now
      maxPlayers: participantCount,
      description: `Epic tournament with ${participantCount} talented musicians competing for the ultimate prize! This is a test tournament with generated participants.`,
      creator: creatorId,
    ...
    // Update selected participants' stats (tournamentsEntered)
    await usersCollection.updateMany(
      { _id: { $in: selectedParticipants } },
      { 
        $inc: { 
    ...
    console.log(`Tournament Creator: testuser01 (${creatorId})`);
    console.log(`Total Available Users: 64`);
    console.log(`Tournament Participants: ${participantCount}`);
...
const createTestTournamentWithSize = async (participantCount) => {
  ...
  try {
    ...
    const usersCollection = db.collection('users');
    const tournamentsCollection = db.collection('tournaments');
      // Get all existing test users (should be 64)
    const existingUsers = await usersCollection.find({
      username: { $regex: /^testuser\d+$/ }
    }).toArray();
    
    if (existingUsers.length < 64) {
      console.log(`❌ Not enough test users found. Need 64 total users, found ${existingUsers.length}`);
      console.log('Run the main script first to create test users.');
      return;
    }
    
    if (participantCount > existingUsers.length) {
      console.log(`❌ Cannot create tournament with ${participantCount} participants. Only ${existingUsers.length} users available.`);
      return;
    }
    ...
    console.log(`📊 Tournament uses ${participantCount} participants out of ${existingUsers.length} available test users`);
    
    return {
      tournamentId: tournamentResult.insertedId,
      participantCount: participantCount,
      totalAvailableUsers: existingUsers.length,
      creatorId: creatorId
    };
