...
const createFakeSubmissions = async (tournament, participants) => {
  ...
  for (const participant of participants) {
    ...
    // Extract YouTube video ID if using YouTube
    let youtubeVideoId;
    if (useYouTube) {
      const match = streamUrl.match(/(?:youtu\.be\/|v=)([^?&]+)/);
      youtubeVideoId = match ? match[1] : undefined;
    }
    const submission = new Submission({
      ...
      youtubeVideoId: useYouTube ? youtubeVideoId : undefined,
...
const beginTournament = async (tournamentId) => {
  console.log('Triggering backend to begin tournament and generate matchups via API...');
  const apiUrl = `http://localhost:${process.env.PORT || 5000}/api/tournaments/${tournamentId}/begin`;
  const response = await fetch(apiUrl, { method: 'POST' });
  if (!response.ok) {
    const errorText = await response.text();
    console.error('Failed to begin tournament:', response.status, errorText);
    throw new Error(`API error: ${response.statusText}`);
  }
  const result = await response.json();
  console.log('Tournament begun, matchups generated by backend');
  return result;
};
