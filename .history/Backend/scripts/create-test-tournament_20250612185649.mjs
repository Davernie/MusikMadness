...
const loadModels = async () => {
  try {
    ...
    if (!User || typeof User.findOne !== 'function') {
      console.error('User model is not correctly loaded or findOne is missing.');
      console.log('User value:', User);
      console.log('Type of User:', typeof User);
      if (User) {
        console.log('Is User a function (Mongoose model constructor)?', typeof User === 'function');
        console.log('User.modelName:', User.modelName); 
        console.log('typeof User.findOne:', typeof User.findOne);
      }
      throw new Error('Failed to load User model correctly.');
    }
    if (!Tournament || typeof Tournament.findById !== 'function') {
      console.error('Tournament model is not correctly loaded or findById is missing.');
      throw new Error('Failed to load Tournament model correctly.');
    }
    if (!Submission || typeof Submission.find !== 'function') { 
      console.error('Submission model is not correctly loaded or find is missing.');
      throw new Error('Failed to load Submission model correctly.');
    }
...
const createOrFindCreator = async (email) => {
  let creator = await User.findOne({ email });
  
  if (!creator) {
    console.log(`Creator user ${email} not found, creating...`);
    const hashedPassword = await bcrypt.hash('testpassword123', 10);
    ...
    console.log(`✓ Found existing creator: ${creator.username}`);
  }
...
const createTournament = async (creatorEmail, tournamentSize, tournamentName) => {
  ...
  const creator = await createOrFindCreator(creatorEmail);
