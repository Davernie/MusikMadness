...
async function checkSubmissions() {
  try {
    ...
    if (totalSubmissions === 0) {
      ...
      // Get YouTube submissions specifically
    const youtubeSubmissions = await Submission.find({ streamingSource: 'youtube' });
    
    console.log('\n--- YouTube Submissions ---');
    if (youtubeSubmissions.length === 0) {
      console.log('No YouTube submissions found.');
    } else {
      youtubeSubmissions.forEach((submission, index) => {
        console.log(`\nYouTube Submission ${index + 1}:`);
    ...
    // Get all submissions
    const submissions = await Submission.find().limit(5).sort({ submittedAt: -1 });
