...
// Ultra-realistic 800-user simulation over 1 minute
export const options = {
  ...
  thresholds: {
    http_req_duration: ['p(95)<5000'],  // 95% under 5 seconds (very generous)
    http_req_failed: ['rate<0.2'],      // Less than 20% errors (realistic under load)
...
// Power user - multiple actions (1% of traffic)
function simulatePowerUser() {
  // Power users always try to login first
  ...
  if (authToken) {
    ...
    // Power users browse multiple sections
    const sections = ['api/users', 'api/tracks', 'api/tournaments'];
    const sectionsToVisit = Math.floor(Math.random() * 3) + 2; // Visit 2-4 sections
    
    for (let i = 0; i < sectionsToVisit; i++) {
      const section = sections[Math.floor(Math.random() * sections.length)];
      const response = http.get(`${BASE_URL}/${section}`, authHeaders);
      check(response, { 'Power user browsing successful': (r) => r.status === 200 });
      sleep(Math.random() * 3 + 1); // 1-4 seconds per section
    }
    
    // Power users are much more likely to visit tournament pages
    if (Math.random() < 0.7) { // 70% visit matchups
      visitRandomMatchupPage();
      
      // They often visit multiple matchups
      if (Math.random() < 0.6) { // 60% visit another
        sleep(Math.random() * 3 + 2); // Time between songs
        visitRandomMatchupPage();
      }
    }
    
    // Power users might check their profile
    if (Math.random() < 0.8) { // 80% check profile
      const profileResponse = http.get(`${BASE_URL}/api/auth/me`, authHeaders);
      check(profileResponse, { 'Power user profile check': (r) => r.status === 200 });
