...
function getUserBehavior() {
  ...
  if (rand < 0.65) return 'idle_browser';      // 65% just browse and leave
  if (rand < 0.85) return 'casual_browser';    // 20% browse multiple pages
  if (rand < 0.93) return 'member';            // 8% existing members login
  if (rand < 0.97) return 'music_listener';    // 4% focused on music/tournaments
  if (rand < 0.99) return 'new_user';          // 2% sign up
...
export default function() { 
  ...
  // Realistic arrival delay - users don't all hit site at once
  sleep(Math.random() * 8 + 1); // 1-9 seconds (page load, reading, distraction)
  
  // High bounce rate - many users leave immediately
  if (Math.random() < 0.20) { // 20% bounce without doing anything
    sleep(Math.random() * 2 + 0.5);
    return;
  }
  
  // Most users land on tournaments page first
  if (Math.random() < 0.80) { // 80% load main content
  ...
  switch(userType) {
    ...
    case 'member':
    case 'new_user':
      sleep(Math.random() * 12 + 3); // 3-15 seconds
      break;
    case 'music_listener':
      sleep(Math.random() * 20 + 5); // 5-25 seconds (listening time)
      break;
    case 'power_user':
      sleep(Math.random() * 25 + 10); // 10-35 seconds
      break;
  }
}
...
function simulateNewUser() {
  ...
  const signupResponse = http.post(`${BASE_URL}/api/auth/signup`, JSON.stringify(newUser), {
    headers: { 'Content-Type': 'application/json' },
    timeout: '25s', // Longer timeout during signup rush
  });
  
  const signupSuccess = check(signupResponse, {
    'Signup attempted': (r) => r.status >= 200 && r.status < 500,
    'Signup time OK': (r) => r.timings.duration < 20000, // More generous during rush
  });
  
  if (!signupSuccess) errorRate.add(1);
  sleep(Math.random() * 3 + 1); // Shorter wait - excited users
  
  // Higher percentage try to login immediately during viral moment
  if (Math.random() < 0.60 && signupResponse.status === 200) { // 60% try immediate login
    sleep(Math.random() * 1 + 0.5); // Very brief pause
    ...
    if (loginResponse.status === 200) {
      try {
        ...
        // New users immediately explore after signup success
        if (Math.random() < 0.80) { // 80% explore immediately
          const response = http.get(`${BASE_URL}/api/tournaments`, { 
            headers: { 'Authorization': `Bearer ${loginData.token}` },
            timeout: '10s' 
          });
          check(response, { 'New user exploring': (r) => r.status === 200 });
          sleep(Math.random() * 3 + 1);
        }
...
export function setup() {
  console.log('🚀 Starting VIRAL SIGNUP RUSH simulation...');
  console.log(`🎯 Target: ${BASE_URL}`);
  console.log('');
  console.log('📊 VIRAL MOMENT: Everyone trying to sign up at once!');
  console.log('   • 85% New users (massive signup rush)');
  console.log('   • 8% Members (existing users checking the buzz)');
  console.log('   • 4% Casual browsers (curious visitors)');
  console.log('   • 2% Music listeners (checking tournament activity)');
  console.log('   • 1% Power users (heavy activity)');
  console.log('');
  console.log('⏱️  Viral Load Timeline:');
  console.log('   0-15s: 200 users (word spreading)');
  console.log('   15-30s: 400 users (viral pickup)');
  console.log('   30-45s: 600 users (full viral mode)');
  console.log('   45-55s: 800 users (PEAK VIRAL MOMENT)');
  console.log('   55-60s: 200 users (initial wave settling)');
  console.log('');
  console.log('🎭 Viral Signup Behaviors:');
  console.log('   • 5% bounce rate (FOMO keeps people engaged)');
  console.log('   • Fast arrival times (0.5-3.5 seconds)');
  console.log('   • 60% immediate login after signup');
  console.log('   • 80% new users explore immediately');
  console.log('   • Higher database pressure from signups');
  console.log('   • Realistic viral moment stress test');
  console.log('');
  ...
  console.log('🏁 Starting viral signup rush test...');
  console.log('⚠️  This will stress-test your database with concurrent signups!');
  console.log('');
