...
import { ArrowLeft, Music, UploadCloud, Info, Youtube, FileMusic } from 'lucide-react';
import AnimatedBackground from '../components/profile/AnimatedBackground';
import { API_BASE_URL } from '../config/api';
import { useAuth } from '../context/AuthContext';
import { extractYouTubeVideoId, isValidYouTubeUrl, getYouTubeThumbnail } from '../utils/youtube';
import { isValidSoundCloudUrl, getSoundCloudThumbnail } from '../utils/soundcloud';
...
const JoinTournamentPage: React.FC = () => {
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      } else if (submissionType === 'soundcloud') {
        ...
        <div className="bg-gray-800/60 backdrop-blur-md rounded-xl shadow-2xl overflow-hidden border border-white/10">
          <div className="p-6 sm:p-8">
            ...
            {error && (
              ...
              {/* Submission Type Toggle */}              <div>
                <label className={labelStyle}>Submission Type</label>
                <div className="grid grid-cols-3 rounded-lg bg-gray-800/50 p-1 border border-gray-700 gap-1">
                  <button
                    type="button"
                  ...
                  >
                    <svg className="h-4 w-4 mr-1" viewBox="0 0 24 24" fill="currentColor">
                      <path d="M3 13.2c.1-.4.4-.8.8-.8s.7.3.8.8l.2 1.2-.2 1.2c-.1.4-.4.8-.8.8s-.7-.3-.8-.8L3 14.4v-1.2zm1.5-.8c.1-.5.4-.9.9-.9s.8.4.9.9l.1.8-.1.8c-.1.5-.4.9-.9.9s-.8-.4-.9-.9l-.1-.8.1-.8zm1.5-.5c.1-.6.5-1.1 1.1-1.1s1 .5 1.1 1.1l.1 1.3-.1 1.3c-.1.6-.5 1.1-1.1 1.1s-1-.5-1.1-1.1L6 14.4v-2.5zm1.5-.3c.1-.7.6-1.3 1.3-1.3s1.2.6 1.3 1.3v3.2c-.1.7-.6 1.3-1.3 1.3s-1.2-.6-1.3-1.3v-3.2zm1.5-.5c.1-.8.7-1.5 1.5-1.5s1.4.7 1.5 1.5v4.2c-.1.8-.7 1.5-1.5 1.5s-1.4-.7-1.5-1.5v-4.2zm1.5 0c.1-.9.8-1.7 1.7-1.7s1.6.8 1.7 1.7v4.2c-.1.9-.8 1.7-1.7 1.7s-1.6-.8-1.7-1.7v-4.2zm1.5-.8c.1-1 .9-1.9 1.9-1.9s1.8.9 1.9 1.9v5.8c-.1 1-.9 1.9-1.9 1.9s-1.8-.9-1.9-1.9v-5.8zm3.5-1.4c.1-1.1 1-2.1 2.1-2.1s2 1 2.1 2.1v8.6c-.1 1.1-1 2.1-2.1 2.1s-2-1-2.1-2.1V9.9z"/>
                    </svg>
                    SoundCloud
                  </button>
                </div>
