...
const MatchupDetailsPage: React.FC = () => {
  ...
  const refreshStreamUrls = useCallback(async (currentMatchupData: MatchupData) => {
    if (!tournamentId || !matchupId || !currentMatchupData) return;
    ...
    try {
      ...
      if (response.ok) {
        ...
        // Ensure currentMatchupData is the one from the state to avoid stale closures if refresh is called independently
        // However, since it's called right after fetchMatchupData with fresh data, this is okay.
        const newMatchup = { ...currentMatchupData }; // Use the passed, fresh matchup data
        let changed = false;

        const p1Stream = streamDataResponse.streamUrls.player1;
        if (p1Stream && newMatchup.player1.submission) {
          newMatchup.player1.submission.streamUrl = p1Stream.embedUrl || p1Stream.streamUrl; // Prefer embedUrl for YouTube
        ...
        if (p2Stream && newMatchup.player2.submission) {
          newMatchup.player2.submission.streamUrl = p2Stream.embedUrl || p2Stream.streamUrl; // Prefer embedUrl for YouTube
  ...
  }, [tournamentId, matchupId, token]); // setMatchup is not needed here as per React's rules for useCallback if it's a state setter
  ...
  }, [tournamentId, matchupId, token, refreshStreamUrls]); // Added refreshStreamUrls and token to dependency array
