...
const MatchupDetailsPage: React.FC = () => {
  ...
  return (
    <div className="min-h-screen relative overflow-hidden">
      {/* EPIC page-wide winner background overlay */}
      {matchup?.status === 'completed' && matchup?.winnerParticipantId && (
        <>
          <div className="winner-page-glow"></div>
          <div className="victory-page-border"></div>
        </>
      )}
      
      {/* Animated background layers for epic depth */}
      <div className="fixed inset-0 z-0">
        ...
        {/* EPIC winner background effects that cover the whole page */}
        {matchup?.status === 'completed' && matchup?.winnerParticipantId && (
          <>
            <div className="absolute inset-0 bg-gradient-to-r from-yellow-600/5 via-amber-600/10 to-yellow-600/5 animate-gradient bg-[length:400%_400%]"></div>
            <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_center,_var(--tw-gradient-stops))] from-yellow-400/5 via-transparent to-transparent animate-victory-pulse"></div>
            
            {/* Epic floating confetti across entire screen */}
            <div className="confetti-rain" style={{ left: '10%', animationDelay: '0s' }}></div>
            <div className="confetti-rain" style={{ left: '20%', animationDelay: '0.5s', background: '#ffed4e' }}></div>
            <div className="confetti-rain" style={{ left: '30%', animationDelay: '1s', background: '#f59e0b' }}></div>
            <div className="confetti-rain" style={{ left: '40%', animationDelay: '1.5s' }}></div>
            <div className="confetti-rain" style={{ left: '50%', animationDelay: '2s', background: '#ffed4e' }}></div>
            <div className="confetti-rain" style={{ left: '60%', animationDelay: '0.3s', background: '#f59e0b' }}></div>
            <div className="confetti-rain" style={{ left: '70%', animationDelay: '0.8s' }}></div>
            <div className="confetti-rain" style={{ left: '80%', animationDelay: '1.3s', background: '#ffed4e' }}></div>
            <div className="confetti-rain" style={{ left: '90%', animationDelay: '1.8s', background: '#f59e0b' }}></div>
          </>
        )}
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 relative z-10"
           style={{ 
             ...(matchup?.status === 'completed' && matchup?.winnerParticipantId && {
               animation: 'screen-shake 0.8s ease-in-out infinite'
             })
           }}>
        ...
        >
          ...
          <div className="relative p-8">
            ...
            <div className="flex flex-col md:flex-row items-center justify-center mb-8">
              {/* First player container with tournament card styling and winner reactions */}
              <div className={`md:w-[42%] ${matchup.winnerParticipantId === matchup.player1.id ? 'animate-epic-bounce winner-glow' : matchup.winnerParticipantId && matchup.winnerParticipantId !== matchup.player1.id ? 'opacity-60 scale-95' : ''}`}>
                <div 
                  className={`relative overflow-hidden rounded-xl transition-all duration-500 tournament-card-hover backdrop-blur-sm p-6 scale-in
                             ${matchup.winnerParticipantId === matchup.player1.id ? 'ultimate-shimmer epic-text-glow' : ''}`}
                  style={{ 
                    background: 'rgba(15, 15, 20, 0.7)',
                    boxShadow: matchup.winnerParticipantId === matchup.player1.id 
                      ? '0 10px 30px -5px rgba(250, 204, 21, 0.3), 0 0 0 1px rgba(250, 204, 21, 0.2)'
                      : '0 10px 30px -5px rgba(0, 0, 0, 0.3), 0 0 0 1px rgba(6, 182, 212, 0.1)'
                  }}
                >
