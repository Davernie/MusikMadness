...
const MatchupDetailsPage: React.FC = () => {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900 text-white relative overflow-hidden">
      {/* Enhanced Gradient Overlay with Animation - reduced opacity for less blurriness */}
      <div className="absolute inset-0 bg-gradient-to-br from-cyan-500/20 via-purple-500/20 to-fuchsia-500/20 pointer-events-none animate-gradient" />
      
      {/* Animated Background Pattern - adjusted for better visibility */}
      <div className="absolute inset-0 opacity-15">
        <div className="absolute inset-0 bg-[radial-gradient(circle_at_1px_1px,_rgba(255,255,255,0.3)_1px,_transparent_0)] bg-[length:50px_50px]" />
      </div>
      
      {/* Floating Orbs - reduced blur and better positions */}
      <div className="absolute top-32 left-[10%] w-40 h-40 bg-cyan-500/15 rounded-full blur-lg animate-float" style={{ animationDuration: '10s' }} />
      <div className="absolute bottom-40 right-[10%] w-56 h-56 bg-fuchsia-500/15 rounded-full blur-lg" style={{ animation: 'float 12s ease-in-out infinite 1s' }} />
      <div className="absolute top-1/2 left-1/4 w-32 h-32 bg-purple-500/15 rounded-full blur-lg" style={{ animation: 'float 8s ease-in-out infinite 0.5s' }} />
      
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-10 relative z-10">
        {/* Enhanced Back button with hover animations */}
        <button 
          onClick={handleBack}
          className="group flex items-center text-cyan-400 hover:text-cyan-300 transition-all duration-300 mb-10 
                     bg-gray-900/60 backdrop-blur-md rounded-full px-6 py-3 border border-cyan-500/30 
                     hover:border-cyan-400/60 hover:bg-gray-900/80 hover:shadow-lg hover:shadow-cyan-500/30
        ...
        <div className="bg-gray-900/70 backdrop-blur-xl rounded-3xl border border-white/10 shadow-2xl 
                        shadow-cyan-500/20 relative overflow-hidden group">
          {/* Animated border gradient - improved visibility with higher contrast */}
          <div className="absolute inset-0 bg-gradient-to-r from-cyan-500/40 via-fuchsia-500/40 to-cyan-500/40 
                          rounded-3xl opacity-60 animate-gradient bg-[length:200%_200%] pointer-events-none 
                          group-hover:opacity-80 transition-opacity duration-500" />
          
          {/* Inner glow with better contrast */}
          <div className="absolute inset-[1px] bg-gray-900/80 rounded-[calc(1.5rem-1px)] backdrop-blur-xl" />
          
          <div className="relative p-8 md:p-10 z-10">
            {/* Enhanced floating header with particle effect - improved alignment */}
            <div className="text-center mb-16 relative">
              {/* Floating background glow - reduced blur for better clarity */}
              <div className="absolute inset-0 bg-gradient-to-r from-cyan-400/15 to-fuchsia-400/15 rounded-full blur-2xl transform scale-150 animate-float" style={{ animationDuration: '6s' }} />
              
              {/* Particle effects - repositioned for better visibility */}
              <div className="absolute top-0 left-1/3 w-2 h-2 bg-cyan-400 rounded-full animate-ping" style={{ animationDuration: '1.5s', animationDelay: '0.2s' }} />
              <div className="absolute top-10 right-1/3 w-1.5 h-1.5 bg-fuchsia-400 rounded-full animate-pulse" style={{ animationDuration: '3s', animationDelay: '0.7s' }} />
              <div className="absolute -bottom-2 left-1/2 w-2 h-2 bg-purple-400 rounded-full animate-float" style={{ animationDuration: '2s', animationDelay: '1s' }} />
              ...
              <div className="h-1.5 w-40 bg-gradient-to-r from-cyan-500 to-fuchsia-500 mx-auto mt-6 rounded-full 
                           animate-pulse shadow-lg shadow-cyan-500/50" />
              <p className="text-gray-300 mt-5 text-xl opacity-95 font-medium tracking-wide">
                {matchup.tournamentName}
              </p>
            </div>

          <div className="flex flex-col md:flex-row items-center justify-between gap-4 mb-12">
            {/* First player - fixed width for better alignment */}
            <div className="md:w-[42%] w-full max-w-md mx-auto md:mx-0">
              <TrackPlayer 
                track={{
              ...
              {matchup.status === 'active' && matchup.player1.id && (
                ...
                >
                  {isVoting ? 'Submitting...' : `Vote for ${matchup.player1.username}`}
                </button>
              )}
              ...
              {matchup.status === 'active' && matchup.player2.id && (
                <button 
                  ...
                  className={`mt-4 w-full py-3 px-4 bg-gradient-to-r from-fuchsia-500 to-purple-600 
                    ...
                    ${isVoting ? 'opacity-70 cursor-not-allowed' : ''}`}
                >
                  {isVoting ? 'Submitting...' : `Vote for ${matchup.player2.username}`}
                </button>
              )}
