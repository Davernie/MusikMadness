...
const TournamentsPage: React.FC = () => {
  ...
  const selectClass = `block w-full pl-4 pr-10 py-2 bg-gray-700/50 border border-cyan-500/30 rounded-xl 
    ...
    hover:border-cyan-500/50 hover:scale-[1.02] hover:shadow-lg 
    active:scale-[0.98] active:shadow-sm select-text`;

  // Class for select text to handle transitions
  const selectTextClass = `
    whitespace-nowrap inline-block w-full select-none
    transition-all duration-300 ease-in-out
    ${isOpen ? 'opacity-100 translate-x-0' : 'opacity-0 -translate-x-4'}
  `;

  // Wrapper class for select containers to handle width transitions
  const selectWrapperClass = "relative inline-block w-full md:w-48 group transition-all duration-300 ease-in-out overflow-hidden";
                ...
                >
                  {genres.map(genre => (
                    <option 
                      key={genre} 
                      value={genre}
                      className={`bg-gray-800 text-white transition-colors duration-300 hover:bg-gray-700 ${selectTextClass}`}
                    >
                      {genre}
                    </option>
                  ))}
