    ...
    <div className="min-h-screen relative overflow-hidden">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 relative z-10">
        ...
        <div className="bg-gray-900/60 backdrop-blur-xl rounded-2xl border border-white/10 shadow-2xl 
          ...
          <div className="relative p-8">            {/* Header with winner enhancement */}
            <div className={`text-center mb-12 relative ${matchup?.status === 'completed' && matchup?.winnerParticipantId ? 'winner-container' : ''}`}>
              <div className={`absolute inset-0 rounded-full blur-3xl transform scale-150 ${
                matchup?.status === 'completed' && matchup?.winnerParticipantId 
                  ? 'bg-gradient-to-r from-yellow-400/15 via-amber-400/20 to-yellow-400/15' 
                  : 'bg-gradient-to-r from-cyan-400/10 to-fuchsia-400/10'
              }`} />
              <h1 className={`text-xl md:text-5xl font-bold text-white relative z-10 tracking-wider ${
                matchup?.status === 'completed' && matchup?.winnerParticipantId ? 'winner-text' : ''
              }`} style={{ fontFamily: 'crashbow, sans-serif' }}>
                <span className={`bg-clip-text text-transparent ${
                  matchup?.status === 'completed' && matchup?.winnerParticipantId
                    ? 'bg-gradient-to-r from-yellow-200 via-white to-yellow-200'
                    : 'bg-gradient-to-r from-cyan-400 via-white to-fuchsia-400'
                }`}>
            ...
            <div className="flex flex-col md:flex-row items-center justify-center mb-8">              {/* First player container with winner glow effect */}
              <div className={`md:w-[42%] ${matchup.winnerParticipantId === matchup.player1.id ? 'winner-glow' : ''}`}>
                <div 
                  className={`relative overflow-hidden rounded-xl transition-all duration-500 backdrop-blur-sm p-6 
                    ${matchup.winnerParticipantId === matchup.player1.id ? 'winner-container' : ''}
                    hover:transform hover:scale-[1.02]`}
                  style={{ 
                    background: matchup.winnerParticipantId === matchup.player1.id 
                      ? 'rgba(15, 15, 20, 0.8)' 
                      : 'rgba(15, 15, 20, 0.7)',
                    boxShadow: matchup.winnerParticipantId === matchup.player1.id
                ...
                >
                  {/* Border and accent */}
                  <div className="absolute inset-0 border border-white/5 rounded-xl"></div>                  <div 
                    className="absolute top-0 left-0 right-0 h-1 rounded-t-xl"
                    style={{ 
                      background: matchup.winnerParticipantId === matchup.player1.id
                        ? 'linear-gradient(to right, rgba(250, 204, 21, 0.8), rgba(250, 204, 21, 0.4))'
                        : 'linear-gradient(to right, rgba(6, 182, 212, 0.8), rgba(6, 182, 212, 0.4))'
                    }}
                  ></div>
                ...
                {canSelectWinner && matchup.player1.id && (
                  ...
                  <div className="mt-4 w-full winner-container winner-border rounded-xl overflow-hidden">
                    <div className="py-4 px-6 text-center backdrop-blur-sm">
                      <div className="flex items-center justify-center gap-3">
                        <span className="winner-trophy text-2xl">🏆</span>
                        <span className="text-lg font-semibold winner-text text-yellow-200">
                          Champion
                        </span>
                        <span className="winner-trophy text-2xl">🏆</span>
                      </div>
                      <div className="mt-2 text-sm text-yellow-300/80 font-medium">
                        Victory Achieved
                      </div>
                    </div>
                  </div>
              ...
              <div className="md:w-[16%] flex items-center justify-center py-6 md:py-0">
                ...
                {/* Second player container with winner glow effect */}
              <div className={`md:w-[42%] ${matchup.winnerParticipantId === matchup.player2.id ? 'winner-glow' : ''}`}>
                <div 
                  className={`relative overflow-hidden rounded-xl transition-all duration-500 backdrop-blur-sm p-6 
                    ${matchup.winnerParticipantId === matchup.player2.id ? 'winner-container' : ''}
                    hover:transform hover:scale-[1.02]`}
                  style={{ 
                    background: matchup.winnerParticipantId === matchup.player2.id 
                      ? 'rgba(15, 15, 20, 0.8)' 
                      : 'rgba(15, 15, 20, 0.7)',
                    boxShadow: matchup.winnerParticipantId === matchup.player2.id
                      ? '0 10px 30px -5px rgba(250, 204, 21, 0.2), 0 0 0 1px rgba(250, 204, 21, 0.1)'
                      : '0 10px 30px -5px rgba(0, 0, 0, 0.3), 0 0 0 1px rgba(217, 70, 239, 0.1)'
                  }}
                >
                  {/* Border and accent */}
                  ...
                  <div 
                    ...
                    style={{ 
                      background: matchup.winnerParticipantId === matchup.player2.id
                        ? 'linear-gradient(to right, rgba(250, 204, 21, 0.8), rgba(250, 204, 21, 0.4))'
                        : 'linear-gradient(to right, rgba(217, 70, 239, 0.8), rgba(217, 70, 239, 0.4))'
                    }}
                  ></div>
            ...
            {/* Matchup Status */}
            <div className="mt-8 text-center">              {matchup.status === 'completed' && matchup.winnerParticipantId && (
                <div className="winner-announcement rounded-2xl p-8 mb-6 winner-glow">
                  <div className="text-center">
                    <div className="flex items-center justify-center gap-4 mb-4">
                      <span className="winner-trophy text-4xl">🏆</span>
                      <h3 className="text-2xl font-bold winner-text text-yellow-200">
                        Matchup Complete
                      </h3>
                      <span className="winner-trophy text-4xl">🏆</span>
                    </div>
                    ...
                    <div className="space-y-3">
                      ...
                      <div className="inline-block px-6 py-3 rounded-xl winner-container winner-border">
                        <span className="text-2xl font-bold winner-name">
                          {matchup.winnerParticipantId === matchup.player1.id ? matchup.player1.username : matchup.player2.username}
                        </span>
                      </div>
                      <p className="text-yellow-300/80 text-sm font-medium mt-4">
                        ✨ Victory Earned Through Excellence ✨
                      </p>
                    </div>
                  </div>
                </div>
