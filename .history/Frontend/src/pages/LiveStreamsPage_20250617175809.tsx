...
const LiveStreamsPage: React.FC = () => {
  ...
  useEffect(() => {
    const fetchStreamers = async () => {
      try {
        ...
        console.log('🔍 DEBUG: Streamers array:', response.streamers);        // Check for user data in streamers
        response.streamers.forEach(streamer => {
          console.log(`🔍 DEBUG: Streamer ${streamer.name}:`, {
            userId: streamer.userId || 'NO USER ID',
            hasUser: !!streamer.user,
            userType: typeof streamer.user,
            username: streamer.user?.username || 'NO USERNAME',
            userEmail: streamer.user?.email || 'NO EMAIL',
            userBio: streamer.user?.bio || 'NO BIO',
            userLocation: streamer.user?.location || 'NO LOCATION'
          });
        });
  ...
  const convertToStreamData = (streamer: Streamer): StreamData => {    console.log(`🔍 DEBUG: Converting streamer ${streamer.name}:`, {
      originalUser: streamer.user,
      userId: streamer.userId,
      hasProfilePicture: !!streamer.user?.profilePicture,
      userExists: !!streamer.user,
      userUsername: streamer.user?.username || 'NO USERNAME'
    });
    ...
    console.log(`🔍 DEBUG: Converted data for ${streamer.name}:`, {
      hasUser: !!converted.user,
      username: converted.user?.username,
      profilePictureUrl: converted.user?.profilePictureUrl
    });
