      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 relative z-10">
        ...
        <div className="bg-gray-900/60 backdrop-blur-xl rounded-2xl border border-white/10 shadow-2xl 
          ...
          <div className="flex flex-col lg:flex-row items-stretch justify-center gap-8 mb-12">
            {/* First player section with enhanced container */}
            <div className="flex-1 max-w-md mx-auto lg:mx-0">
              <div className="group relative">
                {/* Enhanced background glow for player 1 */}
                <div className="absolute -inset-4 bg-gradient-to-r from-cyan-500/20 to-blue-500/20 rounded-2xl blur-xl opacity-60 group-hover:opacity-80 transition-opacity duration-500" />
                
                {/* TrackPlayer container with glass morphism */}
                <div className="relative bg-gray-900/60 backdrop-blur-xl rounded-xl border border-cyan-500/20 p-1 shadow-2xl shadow-cyan-500/10 group-hover:border-cyan-400/40 transition-all duration-300">
                  <TrackPlayer 
                    track={{
                ...
                {/* Enhanced stream info indicator */}
                {matchup.player1.submission?.audioType === 'r2' && (
                  <div className="mt-3 flex items-center justify-center">
                    <div className="bg-cyan-500/20 backdrop-blur-sm rounded-full px-3 py-1.5 border border-cyan-400/30 flex items-center space-x-2">
                      ...
                      <span className="text-cyan-300 text-sm font-medium">Cloud Streaming</span>
                      {isRefreshingUrls && <span className="text-cyan-400 animate-spin text-sm">⟳</span>}
                    </div>
                  </div>
                )}
                
                {/* Enhanced voting button */}
                {matchup.status === 'active' && matchup.player1.id && (
                  <button 
                    onClick={() => handleVote(matchup.player1.id!)}
                    disabled={isVoting}
                    className={`mt-4 w-full py-3 px-6 bg-gradient-to-r from-cyan-500 to-blue-600 
                      ...
                      border-2 border-cyan-400/30 hover:border-cyan-300/50
                      shadow-lg shadow-cyan-500/20 hover:shadow-cyan-500/40
                      ${isVoting ? 'opacity-70 cursor-not-allowed' : 'hover:shadow-xl'}`}
                  >
                    {isVoting ? (
                ...
                {/* Enhanced winner selection button */}
                {canSelectWinner && matchup.player1.id && (
                  <button 
                    onClick={() => handleSelectWinner(matchup.player1.id!)}
                    disabled={isSelectingWinner}
                    className={`mt-3 w-full py-3 px-6 bg-gradient-to-r from-green-500 to-emerald-600 
                      ...
                      ${isSelectingWinner ? 'opacity-70 cursor-not-allowed' : 'hover:shadow-xl'}`}
                  >
                    {isSelectingWinner ? (
                      <span className="flex items-center justify-center">
                        <svg className="animate-spin -ml-1 mr-3 h-5 w-5" fill="none" viewBox="0 0 24 24">
                ...
                {matchup.winnerParticipantId === matchup.player1.id && (
                  <div className="mt-4 w-full py-4 px-6 bg-gradient-to-r from-yellow-500/30 to-amber-500/30 
                                backdrop-blur-sm border-2 border-yellow-400/60 text-yellow-200 font-bold rounded-xl text-center 
                                shadow-lg shadow-yellow-500/30 relative overflow-hidden">
                    {/* Shimmer effect */}
                    <div className="absolute inset-0 bg-gradient-to-r from-transparent via-yellow-400/20 to-transparent transform -skew-x-12 animate-shimmer" />
                    <div className="relative flex items-center justify-center space-x-3">
                      <span className="text-3xl animate-bounce">🏆</span>
                      <span className="text-lg font-bold">CHAMPION</span>
            ...
            {/* Enhanced VS divider with better positioning */}
            <div className="flex items-center justify-center lg:w-32 my-8 lg:my-0">
              <div className="relative group vs-container">
                {/* Animated outer rings */}
                <div className="absolute inset-0 rounded-full border-2 border-cyan-500/30 animate-ping scale-150 vs-outer-ring" />
                <div className="absolute inset-0 rounded-full border border-fuchsia-500/20 animate-pulse scale-125 delay-500 vs-outer-ring" />
                ...
                <div className="rounded-full bg-gradient-to-r from-cyan-500/90 to-fuchsia-500/90 backdrop-blur-md p-2 
                               shadow-2xl shadow-cyan-500/30 transform group-hover:scale-110 transition-all duration-300 relative">
                  <div className="rounded-full bg-black/80 backdrop-blur-md p-6 md:p-8 border border-white/40 
                    ...
                    <div className="text-5xl md:text-6xl font-bold bg-clip-text text-transparent 
                                  ...
                                  animate-pulse tracking-wider vs-symbol vs-glow">
                      VS
                    </div>
                  </div>
                </div>
                ...
                <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-48 md:w-56 h-48 md:h-56 
                ...
                {(matchup.player1.submission?.audioType === 'r2' || matchup.player2.submission?.audioType === 'r2') && (
                  <button
                    ...
                    className="absolute -bottom-12 left-1/2 transform -translate-x-1/2 px-4 py-2 text-sm 
                             ...
                             transform hover:scale-105 glow-button"
            ...
            {/* Second player section with enhanced container */}
            <div className="flex-1 max-w-md mx-auto lg:mx-0">
              <div className="group relative">
                {/* Enhanced background glow for player 2 */}
                <div className="absolute -inset-4 bg-gradient-to-r from-fuchsia-500/20 to-purple-500/20 rounded-2xl blur-xl opacity-60 group-hover:opacity-80 transition-opacity duration-500" />
                
                {/* TrackPlayer container with glass morphism */}
                <div className="relative bg-gray-900/60 backdrop-blur-xl rounded-xl border border-fuchsia-500/20 p-1 shadow-2xl shadow-fuchsia-500/10 group-hover:border-fuchsia-400/40 transition-all duration-300">
                  <TrackPlayer 
                    track={{
                ...
                {matchup.player2.submission?.audioType === 'r2' && (
                  <div className="mt-3 flex items-center justify-center">
                    <div className="bg-fuchsia-500/20 backdrop-blur-sm rounded-full px-3 py-1.5 border border-fuchsia-400/30 flex items-center space-x-2">
                      <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="text-fuchsia-400">
                        <path d="M17.5 19H9a7 7 0 1 1 6.71-9h1.79a4.5 4.5 0 1 1 0 9Z"/>
                      </svg>
                      <span className="text-fuchsia-300 text-sm font-medium">Cloud Streaming</span>
                      {isRefreshingUrls && <span className="text-fuchsia-400 animate-spin text-sm">⟳</span>}
                    </div>
                  </div>
                )}
                ...
                {matchup.status === 'active' && matchup.player2.id && (
                  <button 
                    ...
                    disabled={isVoting}
                    className={`mt-4 w-full py-3 px-6 bg-gradient-to-r from-fuchsia-500 to-purple-600 
                      hover:from-fuchsia-600 hover:to-purple-700 text-white font-semibold rounded-xl 
                      text-center transform transition-all duration-300 hover:scale-105 
                      border-2 border-fuchsia-400/30 hover:border-fuchsia-300/50
          ...
          <div className="mt-8 text-center">
            {matchup.status === 'completed' && matchup.winnerParticipantId && (
              <div className="bg-gradient-to-r from-yellow-500/10 to-amber-500/10 border border-yellow-400/30 rounded-lg p-4 mb-4"></div>
            ...
            {isCreator && (
              <div className="bg-gradient-to-r from-blue-500/10 to-cyan-500/10 border border-blue-400/30 rounded-lg p-4"></div>
