...
const SoundCloudPlayer: React.FC<SoundCloudPlayerProps> = ({
  ...
  const embedUrl = (() => {
    if (originalUrl) {
      // Create embed URL from the original SoundCloud URL with built-in controls
      ...
      // Fallback to trackId-based embed URL with built-in controls
      return `https://w.soundcloud.com/player/?url=https%3A//api.soundcloud.com/tracks/${trackId}&auto_play=${autoplay}&show_artwork=true&show_comments=false&show_playcount=false&show_user=true&visual=false&color=ff6600&hide_related=true`;
    }
    return '';
  })();
