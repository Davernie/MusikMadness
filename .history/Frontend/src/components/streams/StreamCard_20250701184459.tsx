...
const StreamCard: React.FC<StreamCardProps> = React.memo(({ stream }) => {
  ...
  }, [navigate, stream.user, stream.userId]);  const handleButtonMouseEnter = useCallback((e: React.MouseEvent<HTMLButtonElement>) => {
  ...
  }, [platformConfig.bgColor, platformConfig.color]);
    ...
    >
      ...
      <div className="relative p-4 h-full flex flex-col">
        ...
        <div className="relative overflow-hidden rounded-lg mb-4">
          <div className="w-full h-40 bg-gray-800/50 rounded-lg">
            <img 
              src={stream.thumbnailUrl || `https://picsum.photos/seed/${stream.id}/320/180`} 
              alt={stream.streamTitle}
              className={`w-full h-40 object-cover transform transition-all duration-500 group-hover:scale-105 ${
        ...
        >
          ...
          Watch on {stream.platform.charAt(0).toUpperCase() + stream.platform.slice(1)}        </button>
      </div>
    </div>
    </div>
  );
