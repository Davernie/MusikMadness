...
const SoundCloudPlayer: React.FC<SoundCloudPlayerProps> = ({
  ...
  const [isLoading, setIsLoading] = useState(false);  const [widget, setWidget] = useState<any>(null);
  
  const volumeControlRef = useRef<HTMLDivElement>(null);
  const audioRef = useRef<HTMLAudioElement>(null);
  const iframeRef = useRef<HTMLIFrameElement>(null);  // Load SoundCloud Widget API for iframe volume control
  useEffect(() => {
    if (!window.SC && !getStreamUrl() && showPlayer) {
      ...
      script.onload = () => {
        ...
        // Widget API loaded, set up widget if iframe exists
        setTimeout(() => {
    ...
    } else if (window.SC && !getStreamUrl() && showPlayer && iframeRef.current && !widget) {
      // SC already loaded, just set up widget
      console.log('SC already available, setting up widget');
      setTimeout(() => {
        const iframe = iframeRef.current;
        if (iframe && window.SC) {
          const newWidget = window.SC.Widget(iframe);
          newWidget.bind(window.SC.Widget.Events.READY, () => {
            console.log('SoundCloud widget ready (existing SC)');
            setWidget(newWidget);
            newWidget.setVolume(volume * 100);
          });
        }
  ...
  // Update iframe volume when volume changes
  useEffect(() => {
    if (widget && !getStreamUrl()) {
      console.log('Setting widget volume to:', volume * 100);
      widget.setVolume(volume * 100);
    }
  }, [widget, volume]);// Don't render if no valid trackId and no originalUrl
  ...
  const handleVolumeChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // Also update widget if available
    if (widget && !getStreamUrl()) {
      console.log('Updating widget volume immediately:', newVolume * 100);
                ...
                <div className="flex items-center gap-2 flex-1 justify-end">
                  <div className="relative" ref={volumeControlRef}>
                    ...
                    >
                      ...
                      <div className="absolute bottom-full right-0 mb-2 p-4 bg-gray-800/95 backdrop-blur-sm rounded-lg shadow-xl border border-gray-600/50 w-36" style={{ zIndex: 9999 }}>
                        <input
                          type="range"
                          min="0"
                          max="1"
                        ...
                        />
                        <div className="text-xs text-gray-200 text-center mt-2 font-medium">
            ...
            <div className="absolute inset-0 z-10 pointer-events-none">
              <div className="absolute top-2 right-2 pointer-events-auto">
                <div className="relative" ref={volumeControlRef}>
                  <button
                    onMouseDown={(e) => e.stopPropagation()} // Prevent click outside from firing
                    onClick={toggleVolumeControl} // Simplified onClick
                  ...
                  {showVolumeControl && (
                    <div className="absolute bottom-full right-0 mb-2 p-4 bg-gray-800 rounded-lg shadow-xl border border-gray-600 w-36" style={{ zIndex: 9999 }}>
                      <input
                        type="range"
                        min="0"
                        max="1"
