...
const YouTubePlayer: React.FC<YouTubePlayerProps> = ({
  ...
  // Debug logging - keep if helpful, or remove if too verbose
  useEffect(() => {
    console.log('YouTubePlayer props update:', { videoId, title, thumbnail, duration, autoplay, showControls, showPlayer, isAPIReady });
  }, [videoId, title, thumbnail, duration, autoplay, showControls, showPlayer, isAPIReady]);
    ...
    if (shouldPlayerBeActive) {
      ...
      const newPlayerInstance = new window.YT.Player(playerId, {
        ...
        playerVars: {
          ...
          rel: 0,
          enablejsapi: 1,
          origin: window.location.origin, // Added origin
        },
        events: {
          onReady: (event: any) => {
            if (autoplay) {
  ...
  }, [isAPIReady, showPlayer, videoId, autoplay, showControls, onPlay, onPause]); // Dependencies
