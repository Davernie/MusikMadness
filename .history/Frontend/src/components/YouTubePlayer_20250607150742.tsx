...
const YouTubePlayer: React.FC<YouTubePlayerProps> = React.memo(({
  ...
  const [isLoading, setIsLoading] = useState(false);  const playerRef = useRef<any>(null);
  const containerRef = useRef<HTMLDivElement>(null);
  const playerIdRef = useRef<string>('');
  const isInitializedRef = useRef<boolean>(false);
  // Generate stable player ID only once per video
  useEffect(() => {
    playerIdRef.current = `youtube-player-${++playerIdCounter}-${videoId.slice(0, 8)}`;
    isInitializedRef.current = false; // Reset initialization when video changes
  }, [videoId]);
  ...
  useEffect(() => {
    ...
    // Don't recreate if already initialized for the same video
  ...
  }, [isAPIReady, showPlayer, videoId]); // Only recreate when video changes
  // Handle autoplay and controls changes without recreating the player
  useEffect(() => {
    if (playerRef.current && isAPIReady && isInitializedRef.current) {
      try {
        ...
        console.warn('Error updating player settings:', error);
      }
    }
  }, [autoplay, showControls, isAPIReady, isPlaying]);
