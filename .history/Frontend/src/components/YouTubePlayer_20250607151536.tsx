...
const YouTubePlayer: React.FC<YouTubePlayerProps> = React.memo(({
  ...
  const [isAPIReady, setIsAPIReady] = useState(false);  const [playerError, setPlayerError] = useState(false);
  ...
  const handlePlayClick = () => {
    ...
    if (playerError && playerRef.current) {
      // Try to reload just the video without destroying the player
      try {
        playerRef.current.loadVideoById(videoId);
        setPlayerError(false);
        return;
      } catch (error) {
        console.warn('Failed to reload video:', error);
        // Only recreate player as last resort
        setPlayerError(false);
        setShowPlayer(false);
