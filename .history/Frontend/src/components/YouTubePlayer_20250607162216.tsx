...
class PlayerRateLimit {
  ...
  private static readonly MAX_CREATIONS_PER_MINUTE = 15; // Reduced from 30 for better performance
  private static readonly MIN_CREATION_INTERVAL = 100; // Minimum 100ms between creations
...
}>(({ videoId, thumbnail, title, onImageError }) => (
  <img
...
const YouTubePlayer: React.FC<YouTubePlayerProps> = React.memo(({
  ...
  // Memoized thumbnail URL fallback handler
  ...
  const debouncedPlayClick = useMemo(
    () => debounce(() => {
      ...
      setTimeout(() => setIsClickDisabled(false), 800); // Reduced from 1000ms
  ...
  ), [className, debouncedPlayClick, videoId, thumbnail, title, handleImageError, duration, openInYouTube, isClickDisabled]);
