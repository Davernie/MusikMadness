...
import defaultAvatar from '../../assets/images/default-avatar.png'; // Import the default avatar
...
interface TrackPlayerProps {
  track: {
    ...
    audioType?: 'r2' | 'local' | 'youtube' | 'soundcloud'; // Added YouTube and SoundCloud support
...
const TrackPlayer: React.FC<TrackPlayerProps> = ({ track, competitorId, competitorProfileImage, isLeft, gradientStart, gradientEnd, onUrlRefreshNeeded }) => {  // Debug logging for YouTube tracks
  useEffect(() => {
    if (track.audioType === 'youtube') {
      console.log('TrackPlayer YouTube debug:', {
      ...
      console.log('TrackPlayer SoundCloud debug:', {
        audioType: track.audioType,
  ...
  useEffect(() => {
    const handleExclusivePlay = (e: CustomEvent) => {
      ...
      if (id !== track.id && isPlaying) {
        if (track.audioType === 'youtube' || track.audioType === 'soundcloud') {
          // For YouTube and SoundCloud, we'll rely on the callback system to coordinate
          setIsPlaying(false);
    ...
    const togglePlay = () => {    if (error) {
      // If there was an error, try reloading
      setError(null);
      if (track.audioType === 'youtube' || track.audioType === 'soundcloud') {
        // For YouTube and SoundCloud, we'll let the player components handle reload
        return;
      } else if (audioRef.current) {
    ...
    if (track.audioType === 'youtube' || track.audioType === 'soundcloud') {
      // Handle YouTube and SoundCloud players - we can't directly control them
      // but we can coordinate with other players through the callback system
      // The actual play/pause will be handled by the player components
      if (!isPlaying) {
        // Tell other players to pause
        AudioContext.pauseAllExcept(track.id);
        // The player component will handle the actual playback
      }
    } else {
      // Handle audio player
      if (audioRef.current) {
  ...
  const updateProgress = () => {
    ...
    return () => {
      ...
      if (isPlaying) {
        if (track.audioType === 'youtube' || track.audioType === 'soundcloud') {
          // For YouTube and SoundCloud, we'll just update our state
          // The player components handle their own cleanup
        } else if (audioRef.current) {
          audioRef.current.pause();
        }
  ...
  // SoundCloud player event handlers
  ...
  return (
    <div className={`flex flex-col items-${isLeft ? 'start' : 'end'} p-6 bg-gray-800/80 backdrop-blur-md rounded-xl border border-${isLeft ? gradientStart : gradientEnd}-500/30 w-full`}>
          ...
          <>
            <div className="flex items-center">
              <Link to={`/profile/${competitorId}`}>
                <img 
                  src={competitorProfileImage || defaultAvatar} // Use imported default avatar
      ...
      {track.audioType === 'youtube' ? (
        ...
        <div className="w-full mt-3">
          <SoundCloudPlayer
            trackId={track.soundcloudTrackId?.toString() || ''}
            title={track.title}
            artworkUrl={track.soundcloudArtwork}
            duration={track.soundcloudDuration}
            username={track.soundcloudUsername}
            className="w-full"
            autoplay={false}
            showControls={true}
            onPlay={handleSoundCloudPlay} // Added onPlay handler
            onPause={handleSoundCloudPause} // Added onPause handler
          />
        </div>
      ) : (
        <audio 
