...
const BracketMatch: React.FC<BracketMatchProps> = ({ player1, player2, matchupClass, matchupId }) => {
  ...
  const getTeamStateClass = (isWinner: boolean, isTopPlayer: boolean) => {
    ...
    if (isActive) {
      // Use different classes for top and bottom participants in active matchups
      const activeClass = isTopPlayer ? 'activeTop' : 'activeBottom';
      return currentStyles[activeClass] || currentStyles.active || '';
    }
