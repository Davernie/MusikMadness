...
const mockTournamentData = {
  region1: {
    round1: Array.from({ length: 8 }, (_, i) => ({
      player1: { name: `Player ${i * 2 + 1}`, score: Math.floor(Math.random() * 40) + 60 },
      player2: { name: `Player ${i * 2 + 2}`, score: Math.floor(Math.random() * 40) + 60 }
    })),
    round2: Array.from({ length: 4 }, (_, i) => ({
      player1: { name: 'Winner R1 Match ' + (i * 2 + 1), score: Math.floor(Math.random() * 40) + 60 },
      player2: { name: 'Winner R1 Match ' + (i * 2 + 2), score: Math.floor(Math.random() * 40) + 60 }
    })),
    round3: Array.from({ length: 2 }, (_, i) => ({
      player1: { name: 'Winner R2 Match ' + (i * 2 + 1), score: Math.floor(Math.random() * 40) + 60 },
      player2: { name: 'Winner R2 Match ' + (i * 2 + 2), score: Math.floor(Math.random() * 40) + 60 }
    })),
    final: {
      player1: { name: 'Winner R3 Match 1', score: 0 },
      player2: { name: 'Winner R3 Match 2', score: 0 }
    }
  }
};
