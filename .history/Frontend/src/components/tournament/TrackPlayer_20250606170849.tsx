...
interface TrackPlayerProps {
  track: {
    ...
    audioType?: 'r2' | 'local' | 'youtube'; // Added YouTube support
...
const TrackPlayer: React.FC<TrackPlayerProps> = ({ track, competitorId, competitorProfileImage, isLeft, gradientStart, gradientEnd, onUrlRefreshNeeded }) => {
  ...
  const [youtubePlayer, setYoutubePlayer] = useState<any>(null);    // Reset player when track changes
  useEffect(() => {
  ...
  }, [track.id, track.audioUrl, track.youtubeVideoId, youtubePlayer, track.youtubeDuration]);
  ...
  useEffect(() => {
    const handleExclusivePlay = (e: CustomEvent) => {
      ...
      if (id !== track.id && isPlaying) {
        if (track.audioType === 'youtube' && youtubePlayer) {
          youtubePlayer.pauseVideo();
  ...
  }, [track.id, isPlaying, youtubePlayer, track.audioType]);
    const togglePlay = () => {
    if (error) {
  ...
  }, [isPlaying, track.id, track.audioType, youtubePlayer]);
