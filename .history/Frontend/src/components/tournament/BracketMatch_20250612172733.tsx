...
interface BracketMatchProps {
  player1: { name: string; score: number; id: string | null; username?: string };
  player2: { name: string; score: number; id: string | null; username?: string };
  matchupClass: string;
  matchupId?: string;
}
...
const BracketMatch: React.FC<BracketMatchProps> = ({ player1, player2, matchupClass, matchupId }) => {
  ...
  // Safely get player names with fallbacks
  const player1Name = player1.username || player1.username || 'Unknown';
  const player2Name = player2.username || player2.username || 'Unknown';
