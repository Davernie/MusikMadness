...
// Define TournamentColors interface
interface TournamentColors {
  primary: string; // e.g., "0,204,255"
  secondary: string; // e.g., "255,0,255"
  accent: string; // e.g., "0,204,255"
}

// Updated MatchupProps to include colors
interface MatchupProps {
  player1: { name: string; score: number };
  player2: { name: string; score: number };
  matchupClass: string;
  colors: TournamentColors; // Added colors prop
}

const BracketMatch: React.FC<MatchupProps> = ({ player1, player2, matchupClass, colors }) => {
  // Define inline styles for the matchup container to set CSS variables
  const matchupStyle = {
    '--match-accent-color': `rgb(${colors.accent})`,
    '--match-primary-color-rgb': colors.primary, // Store RGB string for rgba() usage
...
// Define TournamentBracketProps
interface TournamentBracketProps {
  title: string;
  colors: TournamentColors;
}

const TournamentBracket: React.FC<TournamentBracketProps> = ({ title, colors }) => {
  // Define inline styles for the main bracket container to set CSS variables for global bracket theme
  const bracketGlobalStyle = {
    '--bracket-primary-color-rgb': colors.primary,
    '--bracket-secondary-color-rgb': colors.secondary,
    '--bracket-accent-color-rgb': colors.accent,
    // For elements that need direct rgb()
    '--bracket-primary-color': `rgb(${colors.primary})`,
    '--bracket-secondary-color': `rgb(${colors.secondary})`,
    '--bracket-accent-color': `rgb(${colors.accent})`,
  } as React.CSSProperties;
  ...
  return (
    <div className={styles.bracketContainerWrapper} style={bracketGlobalStyle}>
      <h2 className={styles.bracketTitle}>{title} Bracket</h2>
