...
const TrackPlayer: React.FC<TrackPlayerProps> = ({ track, competitorId, competitorProfileImage, isLeft, onUrlRefreshNeeded }) => {  // Debug logging for all track types
  ...
  // Also log immediately what audioType we have to debug button visibility
  console.log('🎵 AudioType check:', {
    audioType: track.audioType,
    willShowButton: track.audioType !== 'youtube' && track.audioType !== 'soundcloud'
  });
          ...
          <>
            <div className="flex items-center">
              ...
              {track.audioType !== 'youtube' && track.audioType !== 'soundcloud' && (                <button 
                  onClick={togglePlay}
                  className="p-3 bg-green-500 text-white rounded-full border-2 border-white relative"
                  style={{ zIndex: 999 }}
                >
          ...
          <>
            <div className="flex items-center">
              ...
              <div className="ml-4 flex-1 min-w-0">
                {/* Competitor Avatar and Name */}
                <div className="flex items-center mb-3">
                  <img 
                    src={competitorProfileImage} 
                    alt="Competitor" 
                    className="w-12 h-12 rounded-full border-2 border-white/20"
                    onError={(e) => {
                      e.currentTarget.src = 'data:image/svg+xml;base64,';
                    }}
                  />
                  <div className="ml-3">
                    <Link 
                      to={`/profile/${competitorId}`}
                      className="block text-white font-semibold hover:text-cyan-400 transition-colors text-lg"
                    >
                      {track.artist}
                    </Link>
                  </div>
                ...
                <div className="space-y-2">
                  <h3 className="text-white font-bold text-xl leading-tight truncate">{track.title}</h3>
                  
                  {/* Duration display */}
                  {(track.youtubeDuration || track.soundcloudDuration || duration) && (
                    <div className="text-white/60 text-sm">
                      Duration: {formatTime(track.youtubeDuration || track.soundcloudDuration || Math.floor(duration))}
                    </div>
                  )}
                </div>

                {/* Progress bar - only show for regular audio files */}
                {track.audioType !== 'youtube' && track.audioType !== 'soundcloud' && duration > 0 && (
                  <div className="mt-4">
                    ...
                    <div 
                      className="w-full h-1.5 bg-gray-600/50 rounded-full cursor-pointer"
                      ...
                      <div 
                        ...
                        style={{
                          ...
                          background: `linear-gradient(to right, ${
                            ...
                            isLeft ? 'rgba(6, 182, 212, 0.4)' : 'rgba(217, 70, 239, 0.4)'
                          })`
                        }}
                      />
                    </div>
                ...
                {track.audioType !== 'youtube' && track.audioType !== 'soundcloud' && (
                  <div className="flex items-center mt-3 space-x-3">
                    <div className="flex items-center space-x-2">
                      ...
                      <div className="w-20 h-1.5 bg-gray-600/50 rounded-full cursor-pointer" onClick={handleVolumeClick}>
                        <div 
                          className="h-full rounded-full transition-all duration-300"
                          style={{
                            width: `${volume * 100}%`,
