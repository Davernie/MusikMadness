...
const TrackPlayer: React.FC<TrackPlayerProps> = ({ track, competitorId, competitorProfileImage, isLeft, gradientStart, gradientEnd, onUrlRefreshNeeded }) => {  // Debug logging for all track types
    ...
    const handleExclusivePlay = (e: CustomEvent) => {
      ...
      if (id !== track.id && isPlaying) {
        if (track.audioType === 'youtube') {
          ...
          // For regular audio, pause directly
          if (audioRef.current) {
            audioRef.current.pause();
          }
          setIsPlaying(false);
  ...
  }, [track.id, isPlaying]);  // Toggle play/pause
  const togglePlay = () => {
    if (track.audioType === 'youtube' || track.audioType === 'soundcloud') {
      // For YouTube and SoundCloud, we don't handle play/pause directly
      return;
    }
    ...
    if (isPlaying) {
      ...
      if (audioRef.current) {
        ...
        // Create new promise to track loading state properly
  ...
  const handleError = () => {
    ...
    // Clear this player from the global context
  ...
  const handleLoadedMetadata = () => {
    if (audioRef.current) {
      ...
      setIsLoading(false); // Set loading false when metadata loads successfully
    }
  };
            ...
            <div className="flex items-center">
              ...
              <Link to={`/profile/${competitorId}`}>
                <img 
                  src={competitorProfileImage || '/src/assets/images/default-avatar.png'} // Use default avatar
                  alt={`${track.artist}'s profile`} // Updated alt text
                  className="h-16 w-16 object-cover rounded-md ml-3 cursor-pointer" // Added cursor-pointer
                />
              </Link>
