...
const SoundCloudPlayer: React.FC<SoundCloudPlayerProps> = ({
  ...
  const embedUrl = (() => {
    if (originalUrl) {
      // Create embed URL from the original SoundCloud URL with full controls
      const params = new URLSearchParams();
      params.append('url', originalUrl);
      params.append('auto_play', autoplay.toString());
      params.append('show_artwork', 'true'); // Show artwork
      params.append('show_comments', 'false');
      params.append('show_playcount', 'true'); // Show play count
      params.append('show_user', 'true'); // Show user information
      params.append('visual', 'false'); 
      params.append('color', 'ff6600');
      params.append('hide_related', 'false'); // Show related tracks for full player experience
      params.append('show_teaser', 'false');
      params.append('buying', 'false');
      params.append('liking', 'true'); // Enable like button
      params.append('download', 'true'); // Enable download if available
      params.append('sharing', 'true'); // Enable sharing
      return `https://w.soundcloud.com/player/?${params.toString()}`;
    } else if (trackId) {
      // Fallback to trackId-based embed URL with full controls
      return `https://w.soundcloud.com/player/?url=https%3A//api.soundcloud.com/tracks/${trackId}&auto_play=${autoplay}&show_artwork=true&show_comments=false&show_playcount=true&show_user=true&visual=false&color=ff6600&hide_related=false&show_teaser=false&buying=false&liking=true&download=true&sharing=true`;
    }
    return '';
  })();
    ...
    <div className={`soundcloud-player ${className}`}>
      <div className="relative bg-gray-900 rounded-lg overflow-hidden">
        {embedUrl ? (
          ...
          <div className="w-full h-[166px] bg-gray-800 flex items-center justify-center">
