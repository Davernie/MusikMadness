...
const PlayerThumbnail = React.memo<{
  ...
  const thumbnailSrc = isVisible ? (thumbnail || getYouTubeThumbnail(videoId, 'high')) : 'data:image/svg+xml;base64,PHN2ZyB3aWR0aD0iMzIwIiBoZWlnaHQ9IjE4MCIgeG1sbnM9Imh0dHA6Ly93d3cudzMub3JnLzIwMDAvc3ZnIj48cmVjdCB3aWR0aD0iMTAwJSIgaGVpZ2h0PSIxMDAlIiBmaWxsPSIjY2NjIi8+PC9zdmc+';
  
  // Debug logging for thumbnail loading
  console.log('YouTube thumbnail debug:', {
    videoId,
    providedThumbnail: thumbnail,
    generatedThumbnail: getYouTubeThumbnail(videoId, 'high'),
  ...
  return (
    <img
      ...
      onLoad={() => console.log('YouTube thumbnail loaded successfully for:', videoId)}
      loading="lazy"
    />
  );
});
...
const YouTubePlayer: React.FC<YouTubePlayerProps> = React.memo(({
  ...
  }), [videoId, autoplay, showControls]);  // Memoized thumbnail URL fallback handler with progressive quality fallback
  const handleImageError = useCallback((e: React.SyntheticEvent<HTMLImageElement>) => {
    const target = e.target as HTMLImageElement;
    const currentSrc = target.src;
    
    console.log('YouTube thumbnail error for videoId:', videoId, 'currentSrc:', currentSrc);
    
    // Try different quality levels in order of preference
    if (currentSrc.includes('hqdefault')) {
      console.log('Trying medium quality fallback for:', videoId);
      target.src = getYouTubeThumbnail(videoId, 'medium');
    } else if (currentSrc.includes('mqdefault')) {
      console.log('Trying default quality fallback for:', videoId);
      target.src = getYouTubeThumbnail(videoId, 'default');
    } else if (currentSrc.includes('default.jpg') && !currentSrc.includes('mqdefault') && !currentSrc.includes('hqdefault')) {
      // If standard default failed, try maxres as a last resort
      console.log('Trying maxres quality fallback for:', videoId);
      target.src = getYouTubeThumbnail(videoId, 'maxres');
    } else {
      console.log('All thumbnail qualities failed for:', videoId, 'using final fallback');
      // Final fallback to a solid color placeholder with YouTube branding
      target.src = 'data:image/svg+xml;base64,';
    }
  }, [videoId]);
