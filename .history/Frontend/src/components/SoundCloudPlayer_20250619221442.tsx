...
const SoundCloudPlayer: React.FC<SoundCloudPlayerProps> = ({
  ...
  const handleVolumeChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    console.log('Volume changed to:', newVolume, 'Widget exists:', !!widget); // Debug log
    setVolume(newVolume);
    
    if (widget) {
      widget.setVolume(newVolume * 100); // SoundCloud expects 0-100
      console.log('Called widget.setVolume with:', newVolume * 100); // Debug log
    }
  };
  ...
  const embedUrl = (() => {
    if (originalUrl) {
      ...
      const url = `https://w.soundcloud.com/player/?url=https%3A//api.soundcloud.com/tracks/${trackId}&auto_play=${autoplay}&show_artwork=false&show_comments=false&show_playcount=false&show_user=false&visual=false&color=ff6600&hide_related=true`;
