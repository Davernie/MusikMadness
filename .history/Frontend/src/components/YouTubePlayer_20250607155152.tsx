...
const YouTubePlayer: React.FC<YouTubePlayerProps> = React.memo(({
  ...
  // Load YouTube API with better ad blocker detection
  useEffect(() => {
    let mounted = true;

    const loadAPI = async () => {
      try {
        // Check if YouTube's main domain is accessible (basic ad blocker test)
        const testImage = new Image();
        testImage.onerror = () => {
          if (mounted) {
            console.warn('YouTube domain may be blocked by ad blocker');
            setPlayerError('Ad blocker detected. YouTube player may not work properly. Please disable ad blocker or use the "Open in YouTube" button.');
          }
        };
        testImage.src = 'https://www.youtube.com/favicon.ico?' + Date.now();
        ...
        if (mounted) {
          setPlayerError('Failed to load YouTube API. Ad blocker may be interfering.');
        }
      }
    };
  ...
  const createPlayer = useCallback(async () => {
    ...
    const shortTimeout = setTimeout(() => {
      if (mountedRef.current && isCreatingPlayer) {
        console.warn('YouTube player taking longer than expected...');
        // Don't show error yet, just a warning
      }
    }, 3000);

    const mainTimeout = setTimeout(() => {
      if (mountedRef.current && isCreatingPlayer) {
        console.warn('YouTube player creation timed out - ad blocker likely blocking');
        setIsLoading(false);
        setIsCreatingPlayer(false);
        setPlayerError('Unable to load YouTube player. This often happens when ad blockers interfere with YouTube. Try disabling your ad blocker or opening the video directly on YouTube.');
      }
    }, 8000); // Longer timeout before giving up

    // Force fallback if still loading after 12 seconds
    const forceTimeout = setTimeout(() => {
      if (mountedRef.current && (isCreatingPlayer || isLoading)) {
        console.error('Force stopping YouTube player creation - showing direct link');
        setIsLoading(false);
        setIsCreatingPlayer(false);
        setPlayerError('YouTube player failed to load. Please use the "Open in YouTube" button below.');
      }
    }, 12000);  // Create player with improved fallback handling and ad blocker detection
  const createPlayer = useCallback(async () => {
    if (!isAPIReady || !showPlayer || !videoId || !containerRef.current || isCreatingPlayer) {
      console.log('Skipping player creation:', { isAPIReady, showPlayer, videoId: !!videoId, container: !!containerRef.current, isCreatingPlayer });
      return;
        ...
        {isLoading && (
          ...
          <div className="absolute inset-0 flex items-center justify-center bg-gray-800/90 z-10">
            <div className="text-center text-white max-w-sm mx-4">
              ...
              <div className="text-xs text-gray-400 mb-3 leading-relaxed">{playerError}</div>
              ...
              {playerError.includes('ad blocker') && (
                <div className="text-xs text-gray-500 mt-2">
                  💡 Tip: Try disabling ad blocker for this site
                </div>
              )}
            </div>
          </div>
