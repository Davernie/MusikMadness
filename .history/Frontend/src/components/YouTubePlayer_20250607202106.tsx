...
const PlayerThumbnail = React.memo<{
  ...
  // Default to 'high' quality (hqdefault.jpg)
  const [currentSrc, setCurrentSrc] = useState(thumbnail || getYouTubeThumbnail(videoId, 'high'));
  const [hasError, setHasError] = useState(false);

  // Update src if thumbnail prop changes (e.g. after parent re-fetch)
  useEffect(() => {
    setCurrentSrc(thumbnail || getYouTubeThumbnail(videoId, 'high'));
  ...
  const handleError = useCallback((e: React.SyntheticEvent<HTMLImageElement>) => {
    ...
    // Attempt fallback to 'medium' quality (mqdefault.jpg) if 'high' (hqdefault.jpg) failed
    if (currentSrc.includes('hqdefault.jpg')) {
      const mqSrc = getYouTubeThumbnail(videoId, 'medium');
  ...
  if (hasError) {
    return (
      <div className="w-full h-48 bg-gradient-to-r from-red-500 to-red-700 flex flex-col items-center justify-center text-white rounded-lg">
        <Youtube className="w-10 h-10 mb-2 opacity-80" />
        <div className="text-sm font-semibold">YouTube Video</div>
        <div className="text-xs opacity-90 mt-0.5">Thumbnail unavailable</div>
      </div>
    );
  }
