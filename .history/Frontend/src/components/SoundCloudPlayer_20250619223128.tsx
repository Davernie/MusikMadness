...
import { Play, Pause, ExternalLink, Music, Volume2, VolumeX, SkipBack, SkipForward } from 'lucide-react';
...
const SoundCloudPlayer: React.FC<SoundCloudPlayerProps> = ({
  ...
  const audioRef = useRef<HTMLAudioElement>(null);// Don't render if no valid trackId and no originalUrl
  ...
  const getStreamUrl = () => {
    // For now, we'll use the originalUrl or construct one
    // In a real implementation, you'd need a backend endpoint to get the actual audio stream
    if (originalUrl) {
      // Try to construct a stream URL - this is a placeholder
      return originalUrl.replace('/tracks/', '/stream/') + '?client_id=YOUR_CLIENT_ID';
    }
    return null;
  };
