...
const PlayerThumbnail = React.memo<{
  ...
  const handleError = useCallback((e: React.SyntheticEvent<HTMLImageElement>) => {
    ...
    // Attempt fallback to mqdefault if hqdefault failed
    if (currentSrc.includes('hqdefault.jpg')) {
      const mqSrc = getYouTubeThumbnail(videoId, 'mq');
      console.log('Falling back to mqdefault:', mqSrc);
      setCurrentSrc(mqSrc);
    } else {
      // If mq also fails, or if it wasn't an hq issue, then set final error state
      setHasError(true);
