...
import { Search, Music, Headphones, ChevronDown, ChevronRight, Users, Mic, Music2, Award } from 'lucide-react';
      ...
      <div className="flex flex-col h-full">
        ...
        <nav className="flex-1 px-2 py-4 space-y-2">
          ...
          <div className="space-y-1">
            ...
            >
              <div className="flex items-center">
                ...
                <span className={navTextClass}>Tournaments</span>
              </div>
              {isOpen && (
            ...
            {isOpen && isTournamentOpen && (
              <div className="space-y-1 mt-1">
                <Link to="/tournaments?type=artist" className={dropdownItemClass}>
                  ...
                  <span className={`ml-4 whitespace-nowrap transition-all duration-300 font-crashbow text-sm tracking-wide leading-relaxed ${isOpen ? 'opacity-100 translate-x-0' : 'opacity-0 -translate-x-4'}`}>Artist Tournaments</span>
                </Link>
                <Link to="/tournaments?type=producer" className={dropdownItemClass}>
                  <Music2 className="w-4 h-4 mr-3 flex-shrink-0" />
                  <span className={`ml-4 whitespace-nowrap transition-all duration-300 font-crashbow text-sm tracking-wide leading-relaxed ${isOpen ? 'opacity-100 translate-x-0' : 'opacity-0 -translate-x-4'}`}>Producer Tournaments</span>
                </Link>
                ...
                <Link to="/create-tournament" className={dropdownItemClass}>
                  <ChevronRight className="w-4 h-4 mr-3 flex-shrink-0" />
                  <span className={`ml-4 whitespace-nowrap transition-all duration-300 font-crashbow text-sm tracking-wide leading-relaxed ${isOpen ? 'opacity-100 translate-x-0' : 'opacity-0 -translate-x-4'}`}>Create Tournament</span>
                </Link>
              </div>
            )}
          </div>
          ...
          >
            ...
            <span className={navTextClass}>Leaderboard</span>
          </Link>
          ...
          >
            ...
            <span className={navTextClass}>Create Tournament</span>
          </Link>
        ...
        <div className="p-4 border-t border-gray-700/20">
          <div className={`bg-gray-700/30 rounded-lg p-4 ${isOpen ? 'opacity-100' : 'opacity-0'}`}>
            ...
            <div className="text-xs font-crashbow text-gray-400 mb-2">Summer Beat Battle 2024</div>
        ...
        <div className="p-4">
          <button className="flex items-center w-full px-4 py-3 text-gray-300 hover:text-white transition-colors duration-200 rounded-lg hover:bg-gray-700/20">
            ...
            <span className={`ml-4 whitespace-nowrap transition-all duration-300 font-crashbow ${isOpen ? 'opacity-100 translate-x-0' : 'opacity-0 -translate-x-4'}`}>Logout</span>
          </button>
        </div>
      </div>
    </div>
