...
const YouTubePlayer: React.FC<YouTubePlayerProps> = React.memo(({
  ...
  const createPlayer = useCallback(async () => {
    if (!isAPIReady || !showPlayer || !videoId || !containerRef.current || isCreatingPlayer) {
      console.log('Skipping player creation:', { isAPIReady, showPlayer, videoId: !!videoId, container: !!containerRef.current, isCreatingPlayer });
      return;
    }
  ...
  }, [isAPIReady, showPlayer, videoId, autoplay, showControls, onPlay, onPause, isCreatingPlayer /* Added isCreatingPlayer here */]);

  useEffect(() => {
    // Only attempt to create player if showPlayer is true and API is ready.
    // The createPlayer callback itself has further guards.
    if (showPlayer && isAPIReady) {
        console.log('[useEffect createPlayer] Calling createPlayer for video:', videoId, 'showPlayer:', showPlayer, 'isAPIReady:', isAPIReady);
        createPlayer();
    } else {
        console.log('[useEffect createPlayer] Conditions not met for createPlayer for video:', videoId, 'showPlayer:', showPlayer, 'isAPIReady:', isAPIReady);
    }

    // Cleanup for this effect: if the component unmounts or dependencies change
    // while a player creation is in progress, its timeouts should be cleared.
    // This is primarily handled by creationTimeoutCleanerRef.current() in the main unmount effect
    // and at the start of createPlayer.
    return () => {
        // If createPlayer was called and set up timeouts,
        // and then this effect re-runs (e.g. videoId changes)
        // the new call to createPlayer will clear the previous timeouts.
        // The main unmount effect handles full component unmount.
        console.log('[useEffect createPlayer cleanup] Effect for video', videoId, 'is re-running or unmounting.');
    };
  }, [createPlayer, showPlayer, isAPIReady, videoId]); // Added showPlayer, isAPIReady, videoId to ensure effect runs when these change
  ...
  }, [showPlayer, playerError, isPlaying, createPlayer]);
