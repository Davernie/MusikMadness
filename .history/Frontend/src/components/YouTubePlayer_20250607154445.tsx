...
const YouTubePlayer: React.FC<YouTubePlayerProps> = React.memo(({
  ...
  const createPlayer = useCallback(async () => {
    ...
    const playerTimeout = setTimeout(() => {
      ...
      console.log('Creating YouTube player for video:', videoId, 'with player ID:', playerIdRef.current);      playerRef.current = new window.YT.Player(playerIdRef.current, {
        height: '256',
        width: '100%',
        videoId: videoId,
        playerVars: {
          ...
          playsinline: 1,
          // Simplify parameters to avoid potential issues
          cc_load_policy: 0,
          hl: 'en',
