...
### 8. Scrape Shield (if available)
**Location**: Look for "Scrape Shield" in Security tab or as separate section

#### Available Features (varies by account)
- ✅ **Email Address Obfuscation**: Enable if available
  - Hides email addresses from scrapers
- ✅ **Server-side Excludes**: Enable if available  
  - Hides sensitive content from bots
- ✅ **Hotlink Protection**: Enable if available
  - Prevents other sites from stealing your bandwidth

**Note**: Some Scrape Shield features may not be visible on all free plan accounts.
