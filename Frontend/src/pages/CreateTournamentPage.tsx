...
const SectionTitle = React.memo(({ children, icon: Icon }: { children: React.ReactNode; icon?: any }) => (
  <div 
    style={{
      contain: 'layout style paint',
      transform: 'translate3d(0,0,0)',
...
const CreateTournamentPage: React.FC = () => {  const navigate = useNavigate();
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement | HTMLSelectElement>) => {
    ...
    if (name === 'maxParticipants' || name === 'prizePool' || name === 'entryFee') {
      processedValue = Number(value);
      
      // Enforce 64 participant limit
      if (name === 'maxParticipants' && processedValue > 64) {
        processedValue = 64;
      }
    }
  ...
  const nextStep = () => {
    if (currentStep === 1) {
      ...
      if (formData.maxParticipants > 64) {
        alert('Maximum participants cannot exceed 64.');
        return;
      }
                ...
                {(!token || !user || !user.isCreator) ? (
                  ...
                  <div className="space-y-8">
                    ...
                    <div className="text-center mb-8">
                      <PageTitle>Start Your Music Tournament</PageTitle>
                  ...
                  {currentStep === 1 && (
                    <div className={stepClasses}>
                      <SectionTitle icon={() => <span className="inline-flex items-center justify-center w-10 h-10 rounded-full bg-cyan-500/20 text-cyan-300 mr-3">
                        ...
                        <div className="space-y-6">                          <div>
                          ...
                          <div>
                            ...
                            <input
                              ...
                              className={inputStyle}
                            />
                            <p className="text-xs text-gray-400 mt-1">Maximum of 64 participants allowed</p>
                          </div>
                        </div>
                      </div>
                    </div>
                          ...
                          >
                            <div className="flex items-center mb-6">
                              ...
                              <div className="flex justify-between items-center">
                                ...
                                <span className="text-white">{formatDateForPreview(formData.startDate)}</span>
                              </div>
                              <div className="flex justify-between items-center">
                                <span className="text-gray-400">End Date</span>
                                <span className="text-white">{formatDateForPreview(formData.endDate)}</span>
                              </div>
                              <div className="flex justify-between items-center">
