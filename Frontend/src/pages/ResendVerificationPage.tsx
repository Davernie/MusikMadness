...
const ResendVerificationPage: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!email.trim()) {
      setStatus('error');
      setMessage('Please enter your current email address');
      return;
    }

    if (showEmailChange && newEmail.trim() && newEmail === email) {
    ...
    try {
      ...
      if (showEmailChange && newEmail.trim()) {
        requestBody.newEmail = newEmail;
      }
      ...
      <div className="max-w-md w-full space-y-8">
        <div className="text-center">
          ...
          <h2 className="mt-6 text-3xl font-bold"
            ...
            RESEND VERIFICATION
          </h2>          <p className="mt-2 text-sm text-cyan-400/70">
            Enter your current email address or update it to receive a new verification link
          </p>
        </div>
            ...
            {status !== 'idle' && (
              ...
              <form className="space-y-6" onSubmit={handleSubmit}>
                ...
                <div className="flex items-center">
                  ...
                  />
                  <label htmlFor="change-email" className="ml-2 block text-sm text-cyan-400/70">
                ...
                {showEmailChange && (
                  <div>
                    <label htmlFor="newEmail" className="block text-sm font-medium text-cyan-400/80">
                      New Email Address
                    </label>
                    <div className="mt-1 relative rounded-md shadow-sm">
                      <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                      ...
                      <input
                        ...
                        className={`block w-full pl-10 pr-3 py-2 bg-gray-700/50 border rounded-md focus:outline-none focus:ring-2 text-white placeholder-gray-400 ${
                          fieldErrors.newEmail && fieldErrors.newEmail.length > 0
                            ? 'border-red-500 focus:ring-red-500/50 focus:border-red-500'
                            : 'border-cyan-500/30 focus:ring-cyan-500/50 focus:border-transparent'
                        }`}
                    ...
                    <FieldError fieldName="newEmail" />
                    <p className="mt-1 text-xs text-gray-400">
                      The verification email will be sent to this new address.
                    </p>
                  </div>
                )}
                ...
                <div>
                  ...
                  >
                    {loading ? (
                      ...
                      showEmailChange && newEmail ? 'Update Email & Send Verification' : 'Send Verification Email'
                    )}
                  </button>
                </div>
              </form>
