...
// Memoized Results Count Component to prevent unnecessary re-renders
const ResultsCount = memo(({ 
  ...
  <div 
    className="mb-6 text-cyan-400/80 transform translate3d(0,0,0) will-change-transform contain-layout"
    style={{
      isolation: 'isolate',
  ...
  >
    Showing {processedCount} of {totalCount} tournaments 
    {totalCount > 15 && (
  ...
  return prevProps.processedCount === nextProps.processedCount &&
...
const PageTitle = memo(({ 
  ...
  tournamentTypes: Record<string, string>; 
}) => (
  <h1 
    className="text-4xl font-bold mb-6 text-white transform translate3d(0,0,0) will-change-transform contain-layout contain-style contain-paint"
    style={{ 
  ...
  // Custom comparison function to ensure proper memoization
  return prevProps.tournamentType === nextProps.tournamentType &&
         JSON.stringify(prevProps.tournamentTypes) === JSON.stringify(nextProps.tournamentTypes);
});
  ...
  return (
    <div className="min-h-screen py-12 relative contain-layout">
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 transform translate3d(0,0,0)">
        <div className="mb-10">
          <div 
            className="transform translate3d(0,0,0) contain-layout contain-style"
            style={{
              isolation: 'isolate',
              backfaceVisibility: 'hidden',
              WebkitBackfaceVisibility: 'hidden'
          ...
          {/* Type Navigation */}
          <div className="flex flex-wrap gap-4 mb-8 transform translate3d(0,0,0)">
            <Link 
              to="/tournaments?type=artist"
              className={`px-4 py-2 rounded-full text-sm font-medium transition-all duration-200 will-change-transform ${
          ...
          <ResultsCount 
            processedCount={processedTournaments.length}
          ...
          {/* Tournament Grid */}
          <div className="min-h-[1000px] transform translate3d(0,0,0)"> {/* Reserve minimum height to prevent layout shift */}
            {processedTournaments.length > 0 ? (
              <>              <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-8 max-w-[2000px] mx-auto">
                {processedTournaments.map((tournament) => (
                  <div key={tournament.id} className="will-change-transform contain-layout">
