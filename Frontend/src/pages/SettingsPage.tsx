...
import { Bell, Lock, User, Headphones, Monitor, Loader } from 'lucide-react';
...
const SettingsPage: React.FC = (): JSX.Element => {  
  ...
  useEffect(() => {
    if (user) {
      ...
      // For now, we're not setting a cover image preview as it's not part of the user model
      // We can add this functionality when the backend supports it
    }
  }, [user]);
  ...
  const handleCoverImageChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files && e.target.files[0]) {
      ...
      setCoverImagePreview(result.preview!);
    }
  };
  ...
  const uploadProfileImage = async () => {
    ...
    try {
      ...
      if (user?.avatar) {
        await cleanupOldImage(`${API_URL}/users/profile-picture`, token);
      }
      
      // Upload new image
      const { url, message: uploadMessage } = await uploadImage(
        profileImage,
  ...
  const uploadCoverImage = async () => {
    if (!coverImage || !token) return null;
    
    try {
      // Clean up old image first
      if (user?.avatar) {
        await cleanupOldImage(`${API_URL}/users/cover-image`, token);
      }
  ...
  const handleProfileUpdate = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (coverImage) {
        newCoverImageUrl = await uploadCoverImage();
        if (!newCoverImageUrl) {
      ...
      const response = await fetch(`${API_URL}/users/profile`, {
        ...
        body: JSON.stringify({
          ...
          ...(newProfileImageUrl && { avatar: newProfileImageUrl }),
          ...(newCoverImageUrl && { avatar: newCoverImageUrl })
        })
      });
                ...
                <div className="relative space-y-6">
                  <div className="flex flex-col md:flex-row items-start md:items-center space-y-4 md:space-y-0 md:space-x-4 mb-6">
                    <div className="w-24 h-24 rounded-xl overflow-hidden bg-gradient-to-br from-blue-500/20 to-pink-500/20 backdrop-blur-sm border border-white/10">
                      <img
                        src={profileImagePreview || "https://api.dicebear.com/7.x/avataaars/svg?seed=John"}
                        alt="Profile"
                  ...
                  <div className="pt-4">
                    <button 
                      type="submit" 
                      disabled={loading}
                      className="w-full bg-gradient-to-r from-blue-500/80 to-pink-500/80 hover:from-blue-500 hover:to-pink-500 px-6 py-2 rounded-lg font-medium transition-all duration-300 backdrop-blur-sm flex items-center justify-center"
                    >
                      {loading ? (
                        <>
