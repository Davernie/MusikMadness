...
const SoundCloudPlayer: React.FC<SoundCloudPlayerProps> = ({
  ...
  const openInSoundCloud = () => {
    ...
    const soundcloudUrl = originalUrl || `https://soundcloud.com/track/${trackId}`;
    window.open(soundcloudUrl, '_blank');
  };

  // Create a basic embed URL using the trackId
  const embedUrl = `https://w.soundcloud.com/player/?url=https%3A//api.soundcloud.com/tracks/${trackId}&auto_play=${autoplay}&show_artwork=true&visual=true&color=ff6600`;
